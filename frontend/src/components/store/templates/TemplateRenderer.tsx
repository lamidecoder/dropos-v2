...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: LUXE — Watches/Timepieces (Swiss precision, dark steel + gold)
// ══════════════════════════════════════════════════════════════════════════════
function LuxeTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#B8960C";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#0D0D0D",color:"#E8E0D0",fontFamily:"'Helvetica Neue',Arial,sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="luxe"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .lx-card:hover{transform:translateY(-4px)!important} .lx-card{transition:transform .3s}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(13,13,13,0.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${brand}25`}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:68,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(14px,2vw,18px)",fontWeight:300,letterSpacing:"0.25em",textTransform:"uppercase",color:"#E8E0D0"}}>{store.name}</div>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{padding:"7px 14px",border:`1px solid ${brand}40`,background:"transparent",color:"rgba(232,224,208,0.5)",fontSize:11,letterSpacing:"0.1em",cursor:"pointer",display:"flex",alignItems:"center",gap:6}} onClick={()=>onSearch?.("")}>
              <Search size={12}/> SEARCH
            </div>
            <button onClick={toggle} style={{background:"none",border:`1px solid ${brand}40`,padding:"7px 14px",cursor:"pointer",color:"#E8E0D0",fontSize:11,letterSpacing:"0.1em",display:"flex",alignItems:"center",gap:6}}>
              <ShoppingCart size={12}/> {count>0?`(${count})`:"BAG"}
            </button>
          </div>
        </div>
      </header>
      {/* Hero */}
      <div style={{background:"linear-gradient(135deg,#0D0D0D 0%,#1A1400 100%)",padding:"clamp(60px,10vw,120px) clamp(16px,4vw,32px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,backgroundImage:`radial-gradient(ellipse at 70% 50%,${brand}12,transparent 60%)`,pointerEvents:"none"}}/>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:48,alignItems:"center",position:"relative",zIndex:1}}>
          <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{duration:.8}}>
            <p style={{fontSize:10,letterSpacing:"0.4em",color:brand,textTransform:"uppercase",marginBottom:20,fontWeight:300}}>Swiss Precision · Since 1892</p>
            <h1 style={{fontSize:"clamp(40px,6vw,72px)",fontWeight:200,lineHeight:1.1,margin:"0 0 20px",letterSpacing:"-0.01em"}}>
              Mastery<br/>in Every<br/><span style={{color:brand,fontStyle:"italic",fontWeight:400}}>Second.</span>
            </h1>
            <p style={{fontSize:14,color:"rgba(232,224,208,0.5)",lineHeight:1.9,maxWidth:420,margin:"0 0 36px",fontWeight:300}}>
              {store.description||"Each timepiece is a testament to centuries of horological mastery. For those who understand that time is the ultimate luxury."}
            </p>
            <a href="#collection" style={{display:"inline-flex",alignItems:"center",gap:10,padding:"14px 32px",border:`1px solid ${brand}`,color:brand,textDecoration:"none",fontSize:11,letterSpacing:"0.2em",textTransform:"uppercase",transition:"all .3s"}}
              onMouseEnter={e=>{(e.currentTarget as HTMLElement).style.background=brand;(e.currentTarget as HTMLElement).style.color="#0D0D0D"}}
              onMouseLeave={e=>{(e.currentTarget as HTMLElement).style.background="transparent";(e.currentTarget as HTMLElement).style.color=brand}}>
              View Collection <ArrowRight size={14}/>
            </a>
          </motion.div>
          {w>768&&products[0]&&(
            <motion.div initial={{opacity:0,scale:.95}} animate={{opacity:1,scale:1}} transition={{duration:.8,delay:.2}} style={{position:"relative",display:"flex",justifyContent:"center"}}>
              <div style={{position:"absolute",inset:-20,borderRadius:"50%",background:`radial-gradient(circle,${brand}15,transparent 60%)`,pointerEvents:"none"}}/>
              <img src={products[0].images?.[0]||"https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=500&h=500&fit=crop"} alt={products[0].name}
                style={{width:"80%",maxWidth:360,borderRadius:"50%",objectFit:"cover",aspectRatio:"1",border:`1px solid ${brand}30`,position:"relative",zIndex:1}}/>
            </motion.div>
          )}
        </div>
      </div>
      {/* Products */}
      <div id="collection" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(48px,8vw,80px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:40,borderBottom:`1px solid ${brand}20`,paddingBottom:16}}>
          <h2 style={{fontSize:"clamp(22px,3vw,36px)",fontWeight:200,letterSpacing:"0.05em",margin:0,textTransform:"uppercase"}}>The Collection</h2>
          <span style={{fontSize:11,color:"rgba(232,224,208,0.35)",letterSpacing:"0.1em"}}>{products.length} PIECES</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"240px"},1fr))`,gap:24}}>
          {isLoading?Array.from({length:4}).map((_,i)=><div key={i} style={{background:"rgba(184,150,12,0.05)",aspectRatio:"3/4",borderRadius:4,animation:"pulse 1.5s infinite"}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1523170335258-f5ed11844a49?w=400&h=500&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            return(
              <div key={p.id} className="lx-card">
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <div style={{background:"#1A1400",aspectRatio:"3/4",overflow:"hidden",marginBottom:16,position:"relative"}}>
                    <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",display:"block",filter:"brightness(.95)"}}/>
                    <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(13,13,13,.7),transparent 50%)"}}/>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                      style={{position:"absolute",bottom:16,left:16,right:16,padding:"10px 0",border:`1px solid ${added?brand:"rgba(232,224,208,.3)"}`,background:added?brand:"rgba(13,13,13,.8)",color:added?"#0D0D0D":"#E8E0D0",fontSize:10,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",backdropFilter:"blur(8px)",transition:"all .2s"}}>
                      {added?"ADDED TO BAG":"ADD TO BAG"}
                    </button>
                  </div>
                  <p style={{fontSize:11,color:"rgba(232,224,208,.4)",margin:"0 0 4px",letterSpacing:"0.1em",textTransform:"uppercase"}}>{p.category||"Timepiece"}</p>
                  <p style={{fontSize:15,fontWeight:300,color:"#E8E0D0",margin:"0 0 6px",letterSpacing:"0.02em"}}>{p.name}</p>
                  <p style={{fontSize:14,color:brand,fontWeight:400,letterSpacing:"0.05em"}}>{fmtPrice(p.price,currency)}</p>
                </Link>
              </div>
            );
          })}
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: PAW — Pet Store (Friendly, warm, playful)
// ══════════════════════════════════════════════════════════════════════════════
function PawTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#F97316";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#FFFBF5",fontFamily:"'Nunito',system-ui,sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="paw"/>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Nunito:wght@400;600;700;800;900&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .paw-card:hover{transform:scale(1.03) rotate(-.5deg)} .paw-card{transition:transform .2s cubic-bezier(.34,1.56,.64,1)}`}</style>
      <div style={{background:`${brand}15`,padding:"8px clamp(16px,4vw,32px)",textAlign:"center"}}>
        <p style={{fontSize:13,fontWeight:700,color:brand,margin:0}}>🐾 Free delivery on orders over ₦20,000 · Same-day dispatch</p>
      </div>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(255,251,245,.97)",backdropFilter:"blur(16px)",borderBottom:`2px solid ${brand}20`}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:64,display:"flex",alignItems:"center",gap:16,justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(16px,3vw,22px)",fontWeight:900,color:brand,display:"flex",alignItems:"center",gap:8}}><span>🐾</span>{store.name}</div>
          <div style={{display:"flex",flex:1,maxWidth:280,alignItems:"center",gap:8,padding:"8px 14px",borderRadius:99,background:`${brand}12`,border:`1px solid ${brand}25`}}>
            <Search size={14} color={brand}/>
            <input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Find treats, toys..." style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,color:"#333",fontFamily:"inherit"}}/>
          </div>
          <button onClick={toggle} style={{background:brand,border:"none",borderRadius:99,padding:"9px 18px",cursor:"pointer",color:"#fff",fontWeight:800,fontSize:13,display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}>
            🛒 {count>0?`(${count})`:""} 
          </button>
        </div>
      </header>
      <div style={{background:`linear-gradient(135deg,#FFF3E6 0%,#FFF8F0 100%)`,padding:"clamp(48px,8vw,80px) clamp(16px,4vw,32px)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:32,alignItems:"center"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.6}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"6px 16px",borderRadius:99,background:`${brand}15`,marginBottom:16}}>
              <span style={{fontSize:16}}>⭐</span><span style={{fontSize:12,color:brand,fontWeight:700}}>Nigeria's #1 Pet Store</span>
            </div>
            <h1 style={{fontSize:"clamp(36px,6vw,64px)",fontWeight:900,color:"#1A0F00",lineHeight:1.1,margin:"0 0 16px"}}>
              Everything Your<br/><span style={{color:brand}}>Furry Friend</span><br/>Deserves! 🐶
            </h1>
            <p style={{fontSize:15,color:"rgba(26,15,0,.55)",lineHeight:1.7,margin:"0 0 32px",fontWeight:600,maxWidth:420}}>
              {store.description||"Premium food, toys, accessories, and healthcare for your beloved pets. Delivered fast, with love."}
            </p>
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
              <a href="#products" style={{padding:"14px 32px",borderRadius:99,background:`linear-gradient(135deg,${brand},#E56006)`,color:"#fff",fontSize:15,fontWeight:800,textDecoration:"none",boxShadow:`0 8px 24px ${brand}40`}}>
                Shop Now 🛍️
              </a>
              <a href="#categories" style={{padding:"14px 32px",borderRadius:99,border:`2px solid ${brand}`,color:brand,fontSize:15,fontWeight:700,textDecoration:"none"}}>
                By Pet Type 🐾
              </a>
            </div>
          </motion.div>
          {w>768&&products[0]&&(
            <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} transition={{duration:.6,delay:.2}} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {products.slice(0,4).map((p,i)=>(
                <div key={p.id||i} style={{borderRadius:20,overflow:"hidden",marginTop:i%2===1?20:0,border:`2px solid ${brand}20`,boxShadow:`0 4px 16px ${brand}12`}}>
                  <img src={p.images?.[0]||"https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=300&h=300&fit=crop"} alt={p.name} style={{width:"100%",aspectRatio:"1",objectFit:"cover"}}/>
                </div>
              ))}
            </motion.div>
          )}
      ...
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"20px clamp(16px,4vw,32px)",overflowX:"auto",background:"#FFFBF5"}}>
          <div style={{display:"flex",gap:8,maxWidth:1200,margin:"0 auto",flexWrap:"wrap"}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)}
                style={{padding:"9px 20px",borderRadius:99,border:`2px solid ${category===c?brand:"rgba(249,115,22,.2)"}`,background:category===c?brand:"transparent",color:category===c?"#fff":brand,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div id="products" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(32px,5vw,56px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"220px"},1fr))`,gap:16}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:20,background:`${brand}10`,aspectRatio:"1",animation:"pulse 1.5s infinite"}}/>):
          products.map((p,idx)=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1587300003388-59208cc962cb?w=400&h=400&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            return(
              <div key={p.id} className="paw-card">
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <div style={{borderRadius:20,overflow:"hidden",background:`${brand}08`,border:`2px solid ${brand}20`,boxShadow:`0 4px 16px ${brand}12`}}>
                    <div style={{position:"relative",padding:12,background:`${brand}06`}}>
                      <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"1",objectFit:"cover",borderRadius:14,display:"block"}}/>
                      {idx<3&&<div style={{position:"absolute",top:20,left:20,background:brand,borderRadius:99,padding:"4px 10px",boxShadow:`0 2px 8px ${brand}50`}}>
                        <span style={{fontSize:9,color:"#fff",fontWeight:900}}>⭐ BESTSELLER</span>
                      </div>}
                    </div>
                    <div style={{padding:"12px 14px 14px"}}>
                      <p style={{fontSize:14,fontWeight:800,color:"#1A0F00",margin:"0 0 4px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</p>
                      <p style={{fontSize:11,color:"rgba(26,15,0,.4)",margin:"0 0 10px",fontWeight:600}}>{p.category||"Pet Supplies"}</p>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{fontSize:16,fontWeight:900,color:brand}}>{fmtPrice(p.price,currency)}</span>
                        <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                          style={{padding:"7px 14px",borderRadius:99,background:added?brand:`${brand}15`,border:`2px solid ${added?brand:brand+"40"}`,color:added?"#fff":brand,fontSize:11,fontWeight:800,cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>
                          {added?"Added! 🎉":"Add 🛒"}
                        </button>
                      </div>
                    </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: MONO — Minimal Monochrome (Swiss editorial, black/white)
// ══════════════════════════════════════════════════════════════════════════════
function MonoTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#111111";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#FFFFFF",color:"#111",fontFamily:"'Helvetica Neue','Arial',sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="mono"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .mono-card:hover img{transform:scale(1.04)} .mono-card img{transition:transform .5s ease} .mono-hover:hover{background:#111!important;color:#fff!important}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"#fff",borderBottom:"1px solid #111"}}>
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:56,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(13px,2vw,16px)",fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase"}}>{store.name}</div>
          <div style={{display:"flex",alignItems:"center",gap:0}}>
            {categories.filter(c=>c!=="All").slice(0,4).map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)} style={{padding:"0 16px",height:56,border:"none",borderBottom:category===c?"3px solid #111":"3px solid transparent",background:"transparent",fontSize:12,fontWeight:category===c?700:400,letterSpacing:"0.06em",cursor:"pointer",fontFamily:"inherit",color:"#111",textTransform:"uppercase"}}>
                {c}
              </button>
            ))}
          </div>
          <button onClick={toggle} style={{background:"none",border:"1px solid #111",padding:"6px 16px",cursor:"pointer",fontFamily:"inherit",fontSize:11,fontWeight:700,letterSpacing:"0.1em",display:"flex",alignItems:"center",gap:6}}>
            BAG ({count})
          </button>
        </div>
      </header>
      {/* Full bleed hero */}
      <div style={{position:"relative",height:w>768?"80vh":"60vh",background:"#F0F0F0",overflow:"hidden"}}>
        {products[0]&&<img src={products[0].images?.[0]||"https://images.unsplash.com/photo-1441986300917-64674bd600d8?w=1600&h=900&fit=crop"} alt="" style={{width:"100%",height:"100%",objectFit:"cover",filter:"grayscale(20%)"}}/>}
        <div style={{position:"absolute",inset:0,background:"rgba(0,0,0,0.25)",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"clamp(32px,5vw,64px)"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.7}}>
            <h1 style={{fontSize:"clamp(40px,8vw,96px)",fontWeight:900,color:"#fff",lineHeight:.9,margin:"0 0 20px",letterSpacing:"-0.04em"}}>{store.name}<br/><em style={{fontStyle:"italic",fontWeight:300}}>Studio</em></h1>
            <a href="#products" className="mono-hover" style={{display:"inline-flex",alignItems:"center",gap:10,padding:"12px 28px",border:"2px solid #fff",color:"#fff",textDecoration:"none",fontSize:12,fontWeight:700,letterSpacing:"0.15em",textTransform:"uppercase",transition:"all .2s"}}>
              ENTER COLLECTION <ArrowRight size={14}/>
            </a>
          </motion.div>
        </div>
      </div>
      {/* Products — strict grid */}
      <div id="products" style={{maxWidth:1400,margin:"0 auto"}}>
        <div style={{padding:"20px clamp(16px,4vw,32px)",borderBottom:"1px solid #111",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <span style={{fontSize:12,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase"}}>All Products</span>
          <span style={{fontSize:11,color:"#999",letterSpacing:"0.06em"}}>{products.length} Items</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:w<640?"1fr 1fr":w<900?"repeat(3,1fr)":"repeat(4,1fr)",gap:0}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{aspectRatio:"3/4",background:"#f0f0f0",animation:"pulse 1.5s infinite",borderRight:"1px solid #111",borderBottom:"1px solid #111"}}/>):
          products.map(p=>{
            ...
            const [hover,setHover]=useState(false);
            return(
              <div key={p.id} className="mono-card" style={{borderRight:"1px solid #111",borderBottom:"1px solid #111",position:"relative"}}
                onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <div style={{overflow:"hidden",background:"#f5f5f5"}}>
                    <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"3/4",objectFit:"cover",display:"block",filter:"grayscale(10%)"}}/>
                  </div>
                  <div style={{padding:"14px 16px",borderTop:"1px solid #eee"}}>
                    <p style={{fontSize:13,fontWeight:700,color:"#111",margin:"0 0 2px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"-0.01em"}}>{p.name}</p>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:14,fontWeight:400,color:"#111"}}>{fmtPrice(p.price,currency)}</span>
                      <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                        style={{padding:"5px 12px",background:added?"#111":"transparent",border:"1px solid #111",color:added?"#fff":"#111",fontSize:10,fontWeight:700,letterSpacing:"0.1em",cursor:"pointer",fontFamily:"inherit",transition:"all .15s",textTransform:"uppercase"}}>
                        {added?"✓ ADDED":"+ ADD"}
                      </button>
                    </div>
                  </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: PULSE — Gaming/Esports (Neon red, dark, intense)
// ══════════════════════════════════════════════════════════════════════════════
function PulseTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#FF0040";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#0A0008",color:"#F0E8FF",fontFamily:"'Rajdhani','Impact',sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="pulse"/>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;500;600;700&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}} @keyframes scanH{0%{transform:translateX(-100%)}100%{transform:translateX(100vw)}} .pulse-card:hover{border-color:${brand}!important;transform:translateY(-4px)!important} .pulse-card{transition:all .2s!important}`}</style>
      <div style={{position:"fixed",top:0,left:0,right:0,height:2,background:brand,zIndex:100,boxShadow:`0 0 16px ${brand}`}}/>
      <div style={{position:"fixed",bottom:0,left:0,right:0,height:1,background:`${brand}30`,zIndex:100}}/>
      <header style={{position:"sticky",top:2,zIndex:50,background:"rgba(10,0,8,.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${brand}20`}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:60,display:"flex",alignItems:"center",gap:16,justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(16px,2.5vw,22px)",fontWeight:700,letterSpacing:"0.1em",color:brand,textTransform:"uppercase",textShadow:`0 0 12px ${brand}60`}}>{store.name}</div>
          <div style={{display:"flex",gap:8}}>
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"7px 14px",borderRadius:4,background:`${brand}08`,border:`1px solid ${brand}20`}}>
              <Search size={13} color={brand}/>
              <input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="SEARCH_" style={{background:"transparent",border:"none",outline:"none",fontSize:12,color:"#F0E8FF",fontFamily:"inherit",width:120}}/>
            </div>
            <button onClick={toggle} style={{background:brand,border:"none",padding:"7px 16px",cursor:"pointer",color:"#fff",fontSize:12,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
              BAG [{count}]
            </button>
          </div>
        </div>
      ...
      {/* Hero */}
      <div style={{padding:"clamp(48px,8vw,80px) clamp(16px,4vw,32px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",inset:0,backgroundImage:`linear-gradient(rgba(255,0,64,.03) 1px,transparent 1px),linear-gradient(90deg,rgba(255,0,64,.03) 1px,transparent 1px)`,backgroundSize:"40px 40px",pointerEvents:"none"}}/>
        <div style={{maxWidth:1280,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:48,alignItems:"center",position:"relative",zIndex:1}}>
          <motion.div initial={{opacity:0,x:-24}} animate={{opacity:1,x:0}} transition={{duration:.6}}>
            <div style={{fontSize:10,color:brand,fontFamily:"monospace",marginBottom:20,letterSpacing:"0.1em"}}>// NEXT-GEN GAMING GEAR</div>
            <h1 style={{fontSize:"clamp(48px,9vw,100px)",fontWeight:700,lineHeight:.88,margin:"0 0 20px",textTransform:"uppercase",letterSpacing:"-0.02em"}}>
              PLAY<br/><span style={{color:brand,textShadow:`0 0 32px ${brand}`}}>HARD.</span><br/>WIN<br/>HARDER.
            </h1>
            <p style={{fontSize:14,color:"rgba(240,232,255,.45)",lineHeight:1.7,maxWidth:420,margin:"0 0 36px",fontFamily:"sans-serif"}}>
              {store.description||"Pro-grade gaming peripherals, accessories, and gear for champions. Zero latency. Maximum performance."}
            </p>
            <div style={{display:"flex",gap:10}}>
              <a href="#products" style={{padding:"14px 32px",background:brand,color:"#fff",textDecoration:"none",fontSize:13,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",boxShadow:`0 0 24px ${brand}50`,fontFamily:"inherit"}}>
                SHOP NOW →
              </a>
              <a href="#featured" style={{padding:"14px 32px",border:`1px solid ${brand}40`,color:"rgba(240,232,255,.5)",textDecoration:"none",fontSize:13,fontWeight:600,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"inherit"}}>
                FEATURED
              </a>
            </div>
          </motion.div>
          {w>768&&products[0]&&(
            <motion.div initial={{opacity:0,x:24}} animate={{opacity:1,x:0}} transition={{duration:.6,delay:.2}} style={{position:"relative"}}>
              <div style={{position:"absolute",inset:0,background:`radial-gradient(circle at center,${brand}20,transparent 60%)`,pointerEvents:"none"}}/>
              <img src={products[0].images?.[0]||"https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=600&h=600&fit=crop"} alt={products[0].name}
                style={{width:"100%",borderRadius:4,filter:"saturate(1.2)",position:"relative",zIndex:1}}/>
            </motion.div>
          )}
        </div>
      </div>
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{borderTop:`1px solid ${brand}15`,borderBottom:`1px solid ${brand}15`,overflowX:"auto"}}>
          <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",display:"flex",gap:0}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)}
                style={{padding:"14px 20px",border:"none",borderBottom:category===c?`3px solid ${brand}`:"3px solid transparent",background:"transparent",color:category===c?brand:"rgba(240,232,255,.3)",fontSize:12,fontWeight:700,cursor:"pointer",letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"inherit",whiteSpace:"nowrap",transition:"color .2s"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div id="products" style={{maxWidth:1280,margin:"0 auto",padding:"clamp(32px,5vw,56px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"220px"},1fr))`,gap:12}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:4,background:`${brand}08`,aspectRatio:"1",animation:"pulse 1.5s infinite",border:`1px solid ${brand}15`}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1593305841991-05c297ba4575?w=400&h=400&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            const disc=p.comparePrice&&p.comparePrice>p.price?Math.round((1-p.price/p.comparePrice)*100):0;
            return(
              <div key={p.id} className="pulse-card" style={{borderRadius:4,overflow:"hidden",background:"rgba(255,0,64,.03)",border:`1px solid ${brand}15`}}>
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <div style={{position:"relative"}}>
                    <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"1",objectFit:"cover",display:"block",filter:"saturate(1.1)"}}/>
                    {disc>0&&<div style={{position:"absolute",top:8,left:8,background:brand,padding:"3px 8px",boxShadow:`0 0 8px ${brand}50`}}>
                      <span style={{fontSize:9,color:"#fff",fontWeight:700}}>-{disc}%</span>
                    </div>}
                  </div>
                  <div style={{padding:"12px 14px 14px"}}>
                    <p style={{fontSize:11,color:`${brand}60`,margin:"0 0 4px",letterSpacing:"0.08em",textTransform:"uppercase",fontFamily:"monospace"}}>{p.category||"GAMING"}</p>
                    <p style={{fontSize:14,fontWeight:700,color:"#F0E8FF",margin:"0 0 10px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",textTransform:"uppercase",letterSpacing:"0.02em"}}>{p.name}</p>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:16,fontWeight:700,color:brand,textShadow:`0 0 8px ${brand}40`}}>{fmtPrice(p.price,currency)}</span>
                      <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                        style={{padding:"6px 12px",background:added?brand:"transparent",border:`1px solid ${added?brand:brand+"40"}`,color:added?"#fff":brand,fontSize:10,fontWeight:700,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.08em",textTransform:"uppercase",transition:"all .2s"}}>
                        {added?"✓":"BUY"}
                      </button>
                    </div>
                  </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: BLOOM — Skincare/Wellness (Soft botanical, cream + sage)
// ══════════════════════════════════════════════════════════════════════════════
function BloomTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#7A9E7E";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#FAFAF5",fontFamily:"'DM Sans','Helvetica Neue',sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="bloom"/>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500&family=Canela+Text:ital,wght@0,400;1,400&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .bloom-card:hover{transform:translateY(-6px)!important} .bloom-card{transition:transform .35s ease}`}</style>
      <div style={{background:`linear-gradient(to right,${brand}15,transparent,${brand}10)`,padding:"8px clamp(16px,4vw,32px)",textAlign:"center"}}>
        <p style={{fontSize:12,color:"rgba(26,40,26,.6)",margin:0,fontStyle:"italic"}}>🌿 Clean beauty · Cruelty free · Sustainably sourced</p>
      </div>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(250,250,245,.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${brand}20`}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:64,display:"flex",alignItems:"center",gap:16,justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(17px,2.5vw,22px)",fontWeight:300,color:"#1A281A",letterSpacing:"0.05em",fontStyle:"italic"}}>{store.name}</div>
          <div style={{display:"flex",flex:1,maxWidth:260,alignItems:"center",gap:8,padding:"8px 14px",borderRadius:99,background:`${brand}10`,border:`1px solid ${brand}25`}}>
            <Search size={13} color={brand}/>
            <input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Search products..." style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,color:"#333",fontFamily:"inherit"}}/>
          </div>
          <button onClick={toggle} style={{background:brand,border:"none",borderRadius:99,padding:"9px 18px",cursor:"pointer",color:"#fff",fontSize:13,fontWeight:500,display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}>
            <ShoppingCart size={14}/>{count>0?`(${count})`:"Bag"}
          </button>
        </div>
      </header>
      {/* Hero */}
      <div style={{background:`linear-gradient(135deg,#F0F5E8 0%,#E8F0DF 50%,#F5F0E8 100%)`,padding:"clamp(56px,10vw,100px) clamp(16px,4vw,32px)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:40,alignItems:"center"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.8}}>
            <div style={{display:"inline-flex",alignItems:"center",gap:8,padding:"6px 16px",borderRadius:99,background:"rgba(122,158,126,.15)",border:`1px solid ${brand}30`,marginBottom:20}}>
              <span style={{fontSize:14}}>🌿</span><span style={{fontSize:12,color:brand,fontWeight:500}}>Natural & Effective</span>
            </div>
            <h1 style={{fontFamily:"Georgia,serif",fontSize:"clamp(40px,6vw,72px)",fontWeight:400,color:"#1A281A",lineHeight:1.1,margin:"0 0 20px",letterSpacing:"-0.02em"}}>
              Skin that<br/><em style={{color:brand,fontStyle:"italic"}}>glows</em><br/>naturally.
            </h1>
            <p style={{fontSize:15,color:"rgba(26,40,26,.55)",lineHeight:1.9,maxWidth:440,margin:"0 0 36px",fontWeight:300}}>
              {store.description||"Science-backed skincare rooted in nature. Clean formulas that work with your skin, not against it."}
            </p>
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
              <a href="#products" style={{padding:"14px 32px",borderRadius:99,background:brand,color:"#fff",fontSize:14,fontWeight:500,textDecoration:"none",boxShadow:`0 8px 24px ${brand}35`}}>
                Shop Skincare
              </a>
              <a href="#ritual" style={{padding:"14px 32px",borderRadius:99,border:`1px solid ${brand}40`,color:brand,fontSize:14,fontWeight:400,textDecoration:"none"}}>
                Our Ritual ↗
              </a>
            </div>
          </motion.div>
          {w>768&&(
            <motion.div initial={{opacity:0,x:20}} animate={{opacity:1,x:0}} transition={{duration:.8,delay:.2}} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {products.slice(0,4).map((p,i)=>(
                <div key={p.id||i} style={{borderRadius:20,overflow:"hidden",marginTop:i%2===1?24:0,boxShadow:`0 8px 24px ${brand}15`}}>
                  <img src={p.images?.[0]||"https://images.unsplash.com/photo-1556228720-195a672e8a03?w=300&h=400&fit=crop"} alt={p.name} style={{width:"100%",aspectRatio:i%2===0?"3/4":"1",objectFit:"cover"}}/>
                </div>
              ))}
            </motion.div>
          )}
      ...
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"20px clamp(16px,4vw,32px)",overflowX:"auto"}}>
          <div style={{display:"flex",gap:8,maxWidth:1200,margin:"0 auto"}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)}
                style={{padding:"9px 20px",borderRadius:99,border:`1px solid ${category===c?brand:"rgba(122,158,126,.25)"}`,background:category===c?brand:"transparent",color:category===c?"#fff":brand,fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",transition:"all .2s"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div id="products" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(32px,5vw,56px) clamp(16px,4vw,32px)"}}>
        <div style={{textAlign:"center",marginBottom:40}}>
          <h2 style={{fontFamily:"Georgia,serif",fontSize:"clamp(26px,4vw,40px)",fontWeight:400,color:"#1A281A",margin:"0 0 8px",fontStyle:"italic"}}>Our Bestsellers</h2>
          <p style={{fontSize:14,color:"rgba(26,40,26,.45)",margin:0}}>Loved by thousands of happy customers</p>
        </div>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"220px"},1fr))`,gap:20}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:20,background:`${brand}08`,aspectRatio:"3/4",animation:"pulse 1.5s infinite"}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1556228720-195a672e8a03?w=400&h=500&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            const [wish,setWish]=useState(false);
            const disc=p.comparePrice&&p.comparePrice>p.price?Math.round((1-p.price/p.comparePrice)*100):0;
            return(
              <div key={p.id} className="bloom-card">
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none"}}>
                  <div style={{borderRadius:20,overflow:"hidden",background:"#F0F5E8",position:"relative"}}>
                    <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"3/4",objectFit:"cover",display:"block"}}/>
                    {disc>0&&<div style={{position:"absolute",top:12,left:12,background:brand,borderRadius:99,padding:"3px 10px"}}><span style={{fontSize:10,color:"#fff",fontWeight:500}}>-{disc}%</span></div>}
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();setWish(w=>!w)}}
                      style={{position:"absolute",top:12,right:12,width:32,height:32,borderRadius:"50%",background:"rgba(255,255,255,.9)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center"}}>
                      <Heart size={14} fill={wish?brand:"none"} color={wish?brand:"#999"}/>
                    </button>
                  </div>
                  <div style={{padding:"14px 4px 16px"}}>
                    <p style={{fontSize:12,color:brand,margin:"0 0 4px",fontWeight:500}}>{p.category||"Skincare"}</p>
                    <p style={{fontSize:15,fontWeight:400,color:"#1A281A",margin:"0 0 8px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</p>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:16,fontWeight:700,color:brand}}>{fmtPrice(p.price,currency)}</span>
                      <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                        style={{padding:"7px 16px",borderRadius:99,background:added?brand:"transparent",border:`1px solid ${added?brand:brand+"40"}`,color:added?"#fff":brand,fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>
                        {added?"Added ✓":"Add"}
                      </button>
                    </div>
                  </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: HAVEN — Real Estate/Property (Professional, trust, cream+navy)
// ══════════════════════════════════════════════════════════════════════════════
function HavenTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#1B3A5C";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#F8F5EE",fontFamily:"'Georgia',serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="haven"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .hav-card:hover{transform:translateY(-6px)!important;box-shadow:0 24px 48px rgba(27,58,92,.12)!important} .hav-card{transition:all .35s ease!important}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(27,58,92,.97)",backdropFilter:"blur(20px)"}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:68,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
          <div style={{fontSize:"clamp(16px,2.5vw,20px)",fontWeight:700,color:"#F8F5EE",letterSpacing:"0.02em"}}>{store.name}</div>
          <div style={{display:"flex",flex:1,maxWidth:320,alignItems:"center",gap:8,padding:"9px 16px",borderRadius:4,background:"rgba(248,245,238,.08)",border:"1px solid rgba(248,245,238,.15)"}}>
            <Search size={13} color="rgba(248,245,238,.5)"/>
            <input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Search properties..." style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,color:"#F8F5EE",fontFamily:"inherit"}}/>
          </div>
          <button onClick={toggle} style={{background:"rgba(248,245,238,.1)",border:"1px solid rgba(248,245,238,.2)",borderRadius:4,padding:"9px 18px",cursor:"pointer",color:"#F8F5EE",fontSize:13,fontWeight:600,fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
            <ShoppingCart size={14}/>{count>0?`(${count})`:"Saved"}
          </button>
        </div>
      </header>
      {/* Hero */}
      <div style={{position:"relative",height:w>768?"75vh":"55vh",overflow:"hidden"}}>
        <img src={products[0]?.images?.[0]||"https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=1600&h=900&fit=crop"} alt="" style={{width:"100%",height:"100%",objectFit:"cover",filter:"brightness(.55)"}}/>
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to right,rgba(27,58,92,.7) 30%,transparent)",display:"flex",alignItems:"center",padding:"0 clamp(20px,5vw,64px)"}}>
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.8,delay:.2}} style={{maxWidth:560}}>
            <p style={{fontSize:11,letterSpacing:"0.3em",color:"rgba(248,245,238,.7)",textTransform:"uppercase",marginBottom:16,fontFamily:"'Arial',sans-serif"}}>Premium Properties</p>
            <h1 style={{fontSize:"clamp(40px,6vw,72px)",fontWeight:700,color:"#F8F5EE",lineHeight:1.05,margin:"0 0 20px",letterSpacing:"-0.01em"}}>
              Find Your<br/><em style={{color:"#C8A97E",fontStyle:"italic"}}>Dream</em><br/>Property.
            </h1>
            <p style={{fontSize:15,color:"rgba(248,245,238,.6)",lineHeight:1.7,maxWidth:400,margin:"0 0 32px",fontFamily:"Arial,sans-serif",fontWeight:300}}>
              {store.description||"Premium residential and commercial properties. Your new chapter starts here."}
            </p>
            <a href="#listings" style={{display:"inline-flex",alignItems:"center",gap:10,padding:"14px 32px",borderRadius:4,background:"#C8A97E",color:"#1B3A5C",textDecoration:"none",fontSize:14,fontWeight:700,fontFamily:"Arial,sans-serif",letterSpacing:"0.04em"}}>
      ...
      {/* Products/Listings */}
      <div id="listings" style={{maxWidth:1280,margin:"0 auto",padding:"clamp(40px,6vw,64px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:32}}>
          <h2 style={{fontSize:"clamp(24px,4vw,36px)",fontWeight:700,color:brand,margin:0}}>Featured Listings</h2>
          <span style={{fontSize:13,color:"rgba(27,58,92,.5)",fontFamily:"Arial,sans-serif"}}>{products.length} Properties</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"100%":"300px"},1fr))`,gap:24}}>
          {isLoading?Array.from({length:4}).map((_,i)=><div key={i} style={{borderRadius:8,background:"rgba(27,58,92,.06)",aspectRatio:"3/2",animation:"pulse 1.5s infinite"}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=600&h=400&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            return(
              <div key={p.id} className="hav-card" style={{borderRadius:8,overflow:"hidden",background:"#fff",boxShadow:"0 4px 16px rgba(27,58,92,.06)",border:"1px solid rgba(27,58,92,.08)"}}>
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none"}}>
                  <div style={{position:"relative",overflow:"hidden"}}>
                    <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"3/2",objectFit:"cover",display:"block",transition:"transform .5s"}}/>
                    <div style={{position:"absolute",top:12,left:12,background:brand,color:"#F8F5EE",fontSize:10,fontWeight:700,padding:"4px 10px",borderRadius:2,fontFamily:"Arial,sans-serif",letterSpacing:"0.08em"}}>
                      {p.category||"FOR SALE"}
                    </div>
                  </div>
                  <div style={{padding:"20px"}}>
                    <p style={{fontSize:22,fontWeight:700,color:brand,margin:"0 0 6px",letterSpacing:"-0.01em"}}>{fmtPrice(p.price,currency)}</p>
                    <p style={{fontSize:16,fontWeight:600,color:"#1A1A2E",margin:"0 0 8px"}}>{p.name}</p>
                    <p style={{fontSize:13,color:"rgba(27,58,92,.5)",margin:"0 0 16px",fontFamily:"Arial,sans-serif",lineHeight:1.5}}>{(p.description||"Premium property in a prime location.").slice(0,80)}</p>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                      style={{width:"100%",padding:"11px 0",background:added?brand:"transparent",border:`2px solid ${brand}`,color:added?"#F8F5EE":brand,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",borderRadius:4,transition:"all .2s",letterSpacing:"0.04em"}}>
                      {added?"SAVED ✓":"ENQUIRE NOW"}
                    </button>
                  </div>
                </Link>
              </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: VIBE — Music/Entertainment (Gradient, bold, Gen-Z energy)
// ══════════════════════════════════════════════════════════════════════════════
function VibeTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#8B5CF6";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  const [marquee, setMarquee] = useState(0);
  useEffect(()=>{const t=setInterval(()=>setMarquee(m=>m-1),15);return()=>clearInterval(t);},[]);
  const text = `${store.name.toUpperCase()} · NEW DROPS · LIMITED EDITION · EXCLUSIVE · `;
  return (
    <div style={{minHeight:"100vh",background:"#09090F",color:"#F0E8FF",fontFamily:"'Inter',system-ui,sans-serif",overflowX:"hidden"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="vibe"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.4}} .vib-card:hover{transform:scale(1.03)!important} .vib-card{transition:transform .2s cubic-bezier(.34,1.56,.64,1)!important}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(9,9,15,.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${brand}20`}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:60,display:"flex",alignItems:"center",gap:16,justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(16px,2.5vw,22px)",fontWeight:900,background:`linear-gradient(135deg,${brand},#EC4899)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent",letterSpacing:"-0.03em"}}>{store.name}</div>
          <div style={{display:"flex",flex:1,maxWidth:280,alignItems:"center",gap:8,padding:"8px 14px",borderRadius:99,background:`${brand}10`,border:`1px solid ${brand}20`}}>
            <Search size={13} color={brand}/>
            <input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Search drops..." style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,color:"#F0E8FF",fontFamily:"inherit"}}/>
          </div>
          <button onClick={toggle} style={{background:`linear-gradient(135deg,${brand},#EC4899)`,border:"none",borderRadius:99,padding:"8px 18px",cursor:"pointer",color:"#fff",fontSize:13,fontWeight:700,display:"flex",alignItems:"center",gap:6,fontFamily:"inherit"}}>
            🛒 {count>0?`${count}`:""}
          </button>
        </div>
      </header>
      {/* Marquee */}
      <div style={{background:`linear-gradient(135deg,${brand},#EC4899)`,overflow:"hidden",padding:"8px 0"}}>
        <div style={{display:"flex",whiteSpace:"nowrap",transform:`translateX(${marquee%((text.length)*8)}px)`}}>
          {Array.from({length:8}).map((_,i)=><span key={i} style={{fontSize:12,fontWeight:700,color:"#fff",paddingRight:40,letterSpacing:"0.06em"}}>{text}</span>)}
        </div>
      </div>
      {/* Hero */}
      <div style={{padding:"clamp(48px,8vw,80px) clamp(16px,4vw,32px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:"-50%",left:"-20%",width:600,height:600,borderRadius:"50%",background:`radial-gradient(circle,${brand}20,transparent 60%)`,pointerEvents:"none"}}/>
        <div style={{position:"absolute",bottom:"-30%",right:"-10%",width:400,height:400,borderRadius:"50%",background:"radial-gradient(circle,rgba(236,72,153,.15),transparent 60%)",pointerEvents:"none"}}/>
        <div style={{maxWidth:1280,margin:"0 auto",textAlign:"center",position:"relative",zIndex:1}}>
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:.7}}>
            <h1 style={{fontSize:"clamp(52px,10vw,120px)",fontWeight:900,lineHeight:.88,margin:"0 0 24px",letterSpacing:"-0.05em",background:`linear-gradient(135deg,#F0E8FF,${brand},#EC4899)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
              FEEL THE<br/>VIBE
            </h1>
            <p style={{fontSize:16,color:"rgba(240,232,255,.45)",lineHeight:1.7,maxWidth:480,margin:"0 auto 36px",fontWeight:300}}>
              {store.description||"Exclusive drops, limited runs, and gear for those who set the culture, not follow it."}
            </p>
            <a href="#products" style={{display:"inline-flex",alignItems:"center",gap:8,padding:"14px 36px",borderRadius:99,background:`linear-gradient(135deg,${brand},#EC4899)`,color:"#fff",textDecoration:"none",fontSize:15,fontWeight:700,boxShadow:`0 0 32px ${brand}40`}}>
              SHOP THE DROP ↗
            </a>
          </motion.div>
        </div>
      ...
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"0 clamp(16px,4vw,32px) 24px",overflowX:"auto"}}>
          <div style={{display:"flex",gap:8,maxWidth:1280,margin:"0 auto",justifyContent:"center",flexWrap:"wrap"}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)}
                style={{padding:"9px 20px",borderRadius:99,border:`1px solid ${category===c?brand:"rgba(240,232,255,.1)"}`,background:category===c?`linear-gradient(135deg,${brand},#EC4899)`:"rgba(240,232,255,.03)",color:category===c?"#fff":"rgba(240,232,255,.4)",fontSize:13,fontWeight:600,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",transition:"all .2s"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div id="products" style={{maxWidth:1280,margin:"0 auto",padding:"clamp(24px,4vw,48px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"220px"},1fr))`,gap:16}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:16,background:`${brand}08`,aspectRatio:"1",animation:"pulse 1.5s infinite",border:`1px solid ${brand}12`}}/>):
          products.map((p,idx)=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1511671782779-c97d3d27a1d4?w=400&h=400&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            const colors=["#8B5CF6","#EC4899","#F97316","#10B981","#06B6D4"];
            const c=colors[idx%colors.length];
            return(
              <div key={p.id} className="vib-card">
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <div style={{borderRadius:16,overflow:"hidden",background:`${c}10`,border:`1px solid ${c}20`}}>
                    <div style={{position:"relative",padding:12,background:`${c}08`}}>
                      <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"1",objectFit:"cover",borderRadius:10,display:"block"}}/>
                      {idx<3&&<div style={{position:"absolute",top:20,left:20,background:`linear-gradient(135deg,${brand},#EC4899)`,borderRadius:99,padding:"4px 10px"}}>
                        <span style={{fontSize:9,color:"#fff",fontWeight:700}}>🔥 HOT</span>
                      </div>}
                    </div>
                    <div style={{padding:"12px 14px 14px"}}>
                      <p style={{fontSize:14,fontWeight:800,color:"#F0E8FF",margin:"0 0 4px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</p>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                        <span style={{fontSize:16,fontWeight:900,background:`linear-gradient(135deg,${c},#EC4899)`,WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{fmtPrice(p.price,currency)}</span>
                        <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                          style={{padding:"6px 14px",borderRadius:99,background:added?`linear-gradient(135deg,${brand},#EC4899)`:"transparent",border:`1px solid ${c}40`,color:added?"#fff":c,fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>
                          {added?"✓":"+ Add"}
                        </button>
                      </div>
                    </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: LENS — Photography/Creative (Editorial, dark, portfolio-style)
// ══════════════════════════════════════════════════════════════════════════════
function LensTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#E8D5B0";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#0C0C0C",color:"#E8D5B0",fontFamily:"'Helvetica Neue',Arial,sans-serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="lens"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .lens-card:hover img{transform:scale(1.05)!important;filter:grayscale(0%)!important} .lens-card img{transition:all .5s ease;filter:grayscale(15%)}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(12,12,12,.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(232,213,176,.1)"}}>
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <div style={{fontSize:"clamp(15px,2.5vw,20px)",fontWeight:200,letterSpacing:"0.2em",textTransform:"uppercase",color:"#E8D5B0"}}>{store.name}</div>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <Search size={16} color="rgba(232,213,176,.4)" style={{cursor:"pointer"}}/>
            <button onClick={toggle} style={{background:"none",border:"1px solid rgba(232,213,176,.2)",padding:"7px 16px",cursor:"pointer",color:"#E8D5B0",fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
              CART ({count})
            </button>
          </div>
        </div>
      ...
      {/* Masonry-style hero */}
      <div style={{padding:"clamp(32px,5vw,64px) clamp(16px,4vw,32px) 0"}}>
        <div style={{maxWidth:1400,margin:"0 auto"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.8}} style={{marginBottom:32}}>
            <p style={{fontSize:10,letterSpacing:"0.4em",color:"rgba(232,213,176,.4)",textTransform:"uppercase",marginBottom:16}}>Visual Arts & Photography</p>
            <h1 style={{fontSize:"clamp(48px,9vw,112px)",fontWeight:100,lineHeight:.9,margin:0,letterSpacing:"-0.02em"}}>
              {store.name}<br/><em style={{fontStyle:"italic",fontWeight:300,color:"rgba(232,213,176,.5)"}}>Studio</em>
            </h1>
          </motion.div>
          {/* Masonry grid for hero images */}
          {products.length>0&&(
            <div style={{display:"grid",gridTemplateColumns:w>768?"2fr 1fr 1fr":"1fr 1fr",gap:4,marginBottom:4}}>
              {products.slice(0,w>768?3:2).map((p,i)=>(
                <Link key={p.id} href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block",overflow:"hidden"}} className="lens-card">
                  <img src={p.images?.[0]||"https://images.unsplash.com/photo-1452587925148-ce544e77e70d?w=600&h=500&fit=crop"} alt={p.name}
                    style={{width:"100%",height:i===0?"60vh":w>768?"28vh":"40vw",objectFit:"cover",display:"block"}}/>
                </Link>
              ))}
            </div>
          )}
      ...
      {/* Products */}
      <div id="products" style={{maxWidth:1400,margin:"0 auto",padding:"clamp(32px,5vw,56px) clamp(16px,4vw,32px)"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:24,borderBottom:"1px solid rgba(232,213,176,.1)",paddingBottom:16}}>
          <h2 style={{fontSize:"clamp(18px,3vw,28px)",fontWeight:200,letterSpacing:"0.1em",textTransform:"uppercase",margin:0}}>All Works</h2>
          <span style={{fontSize:11,color:"rgba(232,213,176,.3)",letterSpacing:"0.1em"}}>{products.length} PIECES</span>
        </div>
        <div style={{display:"grid",gridTemplateColumns:w<640?"1fr 1fr":w<900?"repeat(3,1fr)":"repeat(4,1fr)",gap:4}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{background:"rgba(232,213,176,.05)",aspectRatio:"3/4",animation:"pulse 1.5s infinite"}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1452587925148-ce544e77e70d?w=400&h=500&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            const [hover,setHover]=useState(false);
            return(
              <div key={p.id} className="lens-card" style={{position:"relative",overflow:"hidden"}}
                onMouseEnter={()=>setHover(true)} onMouseLeave={()=>setHover(false)}>
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                  <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"3/4",objectFit:"cover",display:"block"}}/>
                  <div style={{position:"absolute",inset:0,background:"rgba(12,12,12,.7)",opacity:hover?1:0,transition:"opacity .3s",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"20px"}}>
                    <p style={{fontSize:14,fontWeight:300,color:"#E8D5B0",margin:"0 0 4px",letterSpacing:"0.02em"}}>{p.name}</p>
                    <p style={{fontSize:15,color:brand,fontWeight:400,margin:"0 0 14px"}}>{fmtPrice(p.price,currency)}</p>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                      style={{padding:"9px 0",border:`1px solid ${added?brand:"rgba(232,213,176,.3)"}`,background:added?brand:"transparent",color:added?"#0C0C0C":"#E8D5B0",fontSize:11,letterSpacing:"0.15em",textTransform:"uppercase",cursor:"pointer",fontFamily:"inherit",transition:"all .2s"}}>
                      {added?"ACQUIRED":"ADD TO CART"}
                    </button>
                  </div>
                </Link>
              </div>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: EMBER — Restaurant/Fine Dining (Warm, appetizing, bold)
// ══════════════════════════════════════════════════════════════════════════════
function EmberDineTemplate({ store, products = [], search, onSearch, category, onCategory, categories = [], isLoading }: TemplateProps) {
  const brand = store.primaryColor || "#C84B2F";
  const currency = store.currency || "NGN";
  const { count, toggle } = useCart(store.id, currency);
  const loading = usePreloader();
  const w = useWindowSize();
  return (
    <div style={{minHeight:"100vh",background:"#1A0E08",color:"#F5EDE0",fontFamily:"'Georgia',serif"}}>
      <StorePreloader brand={brand} storeName={store.name} show={loading} templateId="ember-dine"/>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:.5}} .em-card:hover{transform:translateY(-4px)!important;box-shadow:0 16px 40px rgba(200,75,47,.2)!important} .em-card{transition:all .3s ease!important}`}</style>
      <div style={{background:brand,padding:"8px clamp(16px,4vw,32px)",textAlign:"center"}}>
        <p style={{fontSize:12,color:"#F5EDE0",margin:0,fontStyle:"italic"}}>🔥 Kitchen open 11AM–10PM · Order online for pickup or delivery</p>
      </div>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(26,14,8,.97)",backdropFilter:"blur(20px)",borderBottom:`1px solid ${brand}30`}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",height:68,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
          <div>
            <div style={{fontSize:"clamp(18px,3vw,26px)",fontWeight:700,color:"#F5EDE0",fontStyle:"italic"}}>{store.name}</div>
            <div style={{fontSize:10,letterSpacing:"0.2em",color:"rgba(245,237,224,.4)",textTransform:"uppercase"}}>Restaurant & Bar</div>
          </div>
          <nav style={{display:"flex",gap:"clamp(16px,3vw,32px)",fontFamily:"Arial,sans-serif",fontSize:13,color:"rgba(245,237,224,.5)"}}>
            {w>640&&<><a href="#menu" style={{textDecoration:"none",color:"inherit"}}>Menu</a><a href="#about" style={{textDecoration:"none",color:"inherit"}}>About</a><a href="#contact" style={{textDecoration:"none",color:"inherit"}}>Reserve</a></>}
          </nav>
          <button onClick={toggle} style={{background:brand,border:"none",borderRadius:4,padding:"10px 20px",cursor:"pointer",color:"#F5EDE0",fontFamily:"Arial,sans-serif",fontSize:13,fontWeight:700,letterSpacing:"0.04em",display:"flex",alignItems:"center",gap:6}}>
            <ShoppingCart size={14}/>{count>0?`(${count})`:""} ORDER
          </button>
        </div>
      </header>
      {/* Hero */}
      <div style={{position:"relative",height:w>768?"70vh":"55vh",overflow:"hidden"}}>
        <img src={products[0]?.images?.[0]||"https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=1600&h=900&fit=crop"} alt="" style={{width:"100%",height:"100%",objectFit:"cover",filter:"brightness(.4)"}}/>
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to top,rgba(26,14,8,.95) 0%,transparent 60%)",display:"flex",flexDirection:"column",justifyContent:"flex-end",padding:"clamp(32px,5vw,64px)"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:.8,delay:.2}}>
            <p style={{fontSize:11,letterSpacing:"0.3em",color:brand,textTransform:"uppercase",marginBottom:12,fontFamily:"Arial,sans-serif"}}>Est. 2019 · Lagos, Nigeria</p>
            <h1 style={{fontSize:"clamp(48px,8vw,96px)",fontWeight:700,lineHeight:.92,margin:"0 0 20px",color:"#F5EDE0"}}>
              Where Every<br/>Meal Becomes<br/><em style={{color:brand,fontStyle:"italic"}}>a Memory.</em>
            </h1>
            <div style={{display:"flex",gap:12,flexWrap:"wrap"}}>
              <a href="#menu" style={{padding:"14px 32px",borderRadius:2,background:brand,color:"#F5EDE0",fontSize:14,fontWeight:700,textDecoration:"none",fontFamily:"Arial,sans-serif",letterSpacing:"0.04em"}}>
                VIEW MENU
              </a>
              <a href="#reserve" style={{padding:"14px 32px",borderRadius:2,border:"1px solid rgba(245,237,224,.2)",color:"rgba(245,237,224,.6)",fontSize:14,textDecoration:"none",fontFamily:"Arial,sans-serif"}}>
                RESERVE A TABLE
              </a>
            </div>
          </motion.div>
      ...
      {/* Menu categories */}
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{borderBottom:`1px solid ${brand}20`,overflowX:"auto"}}>
          <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,32px)",display:"flex",gap:0}}>
            {categories.map(c=>(
              <button key={c} onClick={()=>onCategory?.(c)}
                style={{padding:"16px 24px",border:"none",borderBottom:category===c?`3px solid ${brand}`:"3px solid transparent",background:"transparent",color:category===c?brand:"rgba(245,237,224,.35)",fontSize:13,fontWeight:600,cursor:"pointer",letterSpacing:"0.06em",fontFamily:"Arial,sans-serif",whiteSpace:"nowrap",transition:"color .2s",textTransform:"uppercase"}}>
                {c}
              </button>
            ))}
          </div>
      ...
      <div id="menu" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(40px,6vw,64px) clamp(16px,4vw,32px)"}}>
        <div style={{textAlign:"center",marginBottom:48}}>
          <p style={{fontSize:11,letterSpacing:"0.3em",color:brand,textTransform:"uppercase",marginBottom:10,fontFamily:"Arial,sans-serif"}}>Curated Daily</p>
          <h2 style={{fontSize:"clamp(28px,4vw,48px)",fontWeight:700,margin:0,letterSpacing:"-0.01em"}}>Today's Menu</h2>
        </div>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"100%":"280px"},1fr))`,gap:20}}>
          {isLoading?Array.from({length:6}).map((_,i)=><div key={i} style={{borderRadius:8,background:"rgba(200,75,47,.08)",aspectRatio:"4/3",animation:"pulse 1.5s infinite"}}/>):
          products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1414235077428-338989a2e8c0?w=500&h=350&fit=crop";
            const {add}=useCart(store.id,currency);
            const [added,setAdded]=useState(false);
            return(
              <div key={p.id} className="em-card" style={{borderRadius:8,overflow:"hidden",background:"rgba(200,75,47,.06)",border:`1px solid ${brand}20`,boxShadow:"0 4px 16px rgba(200,75,47,.06)"}}>
                <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none"}}>
                  <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"4/3",objectFit:"cover",display:"block"}}/>
                  <div style={{padding:"18px"}}>
                    <p style={{fontSize:12,color:brand,margin:"0 0 6px",fontFamily:"Arial,sans-serif",fontWeight:600,letterSpacing:"0.06em",textTransform:"uppercase"}}>{p.category||"Main Course"}</p>
                    <p style={{fontSize:18,fontWeight:700,color:"#F5EDE0",margin:"0 0 8px",letterSpacing:"-0.01em"}}>{p.name}</p>
                    <p style={{fontSize:12,color:"rgba(245,237,224,.45)",margin:"0 0 16px",lineHeight:1.5,fontFamily:"Arial,sans-serif"}}>{(p.description||"Prepared fresh daily with the finest Nigerian ingredients.").slice(0,70)}</p>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:20,fontWeight:700,color:brand}}>{fmtPrice(p.price,currency)}</span>
                      <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}}
                        style={{padding:"10px 20px",background:added?brand:"transparent",border:`1px solid ${brand}`,color:added?"#F5EDE0":brand,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",borderRadius:4,transition:"all .2s",letterSpacing:"0.04em"}}>
                        {added?"ADDED ✓":"ORDER"}
                      </button>
                    </div>
                  </div>
    ...
    <div style={{minHeight:"100vh",background:"#fff",fontFamily:"'Helvetica Neue',Arial,sans-serif",color:"#111"}}>
      ...
      <header style={{borderBottom:"1px solid #e8e8e8",position:"sticky",top:0,zIndex:50,background:"#fff"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,40px)",height:56,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <div style={{flex:1,maxWidth:320,margin:"0 24px",display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:0,border:"1px solid #e8e8e8"}}>
              ...
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                ...
                <div style={{padding:"12px 14px 16px"}}>
                  ...
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    ...
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{padding:"5px 12px",border:"1px solid #111",background:added?"#111":"transparent",color:added?"#fff":"#111",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",transition:"all 0.15s"}}>{added?"✓":"ADD"}</button>
                  </div>
                </div>
              </Link>
            </div>);
...
function BlazeTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  ...
  return(
    <div style={{minHeight:"100vh",background:"#0D0D0D",color:"#F5F5F5",fontFamily:"'Rajdhani','Impact',sans-serif",overflowX:"hidden"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Rajdhani:wght@400;600;700&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}@keyframes scanl{0%{transform:translateY(-100vh)}100%{transform:translateY(100vh)}}`}</style>
      <div style={{position:"fixed",top:0,left:0,right:0,height:2,background:`linear-gradient(to right,transparent,${brand},transparent)`,animation:"scanl 3s linear infinite",zIndex:99,opacity:0.6,pointerEvents:"none"}}/>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(13,13,13,0.97)",borderBottom:`2px solid ${brand}`}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,3vw,32px)",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          ...
          <button onClick={toggle} style={{background:brand,border:"none",padding:"8px 16px",color:"#F5F5F5",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",display:"flex",alignItems:"center",gap:6,letterSpacing:"0.06em"}}><ShoppingCart size={14}/> [{count}]</button>
        </div>
      </header>
      <div style={{background:"linear-gradient(135deg,#1A0005,#0D0D0D)",padding:"clamp(48px,8vw,96px) clamp(16px,3vw,32px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:0,right:0,width:"40%",height:"100%",background:`linear-gradient(to left,${brand}10,transparent)`,pointerEvents:"none"}}/>
        <div style={{maxWidth:1280,margin:"0 auto",position:"relative",zIndex:1}}>
          <div style={{display:"inline-block",padding:"4px 12px",background:`${brand}20`,border:`1px solid ${brand}40`,marginBottom:16}}><span style={{fontSize:11,color:brand,fontWeight:700,letterSpacing:"0.2em"}}>SEASON 2025</span></div>
          <h1 style={{fontSize:"clamp(52px,10vw,112px)",fontWeight:700,lineHeight:0.85,margin:"0 0 20px",textTransform:"uppercase",letterSpacing:"-0.03em"}}>{store.name.toUpperCase()}<br/><span style={{color:brand,WebkitTextStroke:`2px ${brand}`,WebkitTextFillColor:"transparent"}}>STORE</span></h1>
          <p style={{fontSize:14,color:"rgba(245,245,245,0.5)",maxWidth:480,margin:"0 0 32px"}}>{store.description||"Level up your setup. Premium gaming gear for serious players."}</p>
          <a href="#products" style={{display:"inline-block",padding:"14px 32px",background:brand,color:"#fff",fontSize:15,fontWeight:700,textDecoration:"none",letterSpacing:"0.06em",textTransform:"uppercase"}}>ENTER STORE →</a>
        </div>
      </div>
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{borderBottom:`2px solid ${brand}20`,overflowX:"auto"}}>
          <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,3vw,32px)",display:"flex",gap:0}}>
            {categories.map(c=><button key={c} onClick={()=>onCategory?.(c)} style={{padding:"14px 20px",border:"none",borderBottom:`3px solid ${category===c?brand:"transparent"}`,background:"transparent",color:category===c?brand:"rgba(245,245,245,0.35)",fontSize:12,fontWeight:700,cursor:"pointer",letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"inherit",whiteSpace:"nowrap"}}>{c}</button>)}
          </div>
        </div>
      )}
      <div id="products" style={{maxWidth:1280,margin:"0 auto",padding:"clamp(32px,5vw,56px) clamp(16px,3vw,32px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"220px"},1fr))`,gap:2,background:`${brand}10`}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{background:"#1A1A1A",aspectRatio:"1",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            const img=p.images?.[0]||"https://images.unsplash.com/photo-1587202372634-32705e3bf49c?w=500&h=500&fit=crop";
            const{add}=useCart(store.id,currency); const[added,setAdded]=useState(false);
            return(<div key={p.id} style={{background:"#1A1A1A",position:"relative"}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{position:"relative",aspectRatio:"1",overflow:"hidden"}}>
                  <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",filter:"contrast(1.1)"}}/>
                  <div style={{position:"absolute",inset:0,background:`linear-gradient(to top,rgba(13,13,13,0.8),transparent 50%)`}}/>
                </div>
                <div style={{padding:"12px 14px 14px"}}>
                  <p style={{fontSize:12,color:`${brand}`,margin:"0 0 3px",fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase"}}>{p.category||"GEAR"}</p>
                  <p style={{fontSize:14,fontWeight:700,color:"#F5F5F5",margin:"0 0 10px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",letterSpacing:"0.02em",textTransform:"uppercase"}}>{p.name}</p>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:16,fontWeight:700,color:brand}}>{fmtPrice(p.price,currency)}</span>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{padding:"6px 14px",background:added?"#1A1A1A":brand,border:`1px solid ${brand}`,color:"#F5F5F5",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.06em"}}>{added?"✓ ADDED":"+ ADD"}</button>
                  </div>
                </div>
              </Link>
            </div>);
...
function FloraTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F5F9F0",color:"#1A2E1C",fontFamily:"'Crimson Pro','Georgia',serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Crimson+Pro:ital,wght@0,300;0,400;0,600;1,400&family=DM+Sans:wght@300;400;500&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}.flora-card:hover{transform:translateY(-6px)!important}.flora-card{transition:transform 0.3s ease}`}</style>
      <div style={{background:"#1A2E1C",padding:"8px",textAlign:"center"}}><span style={{fontSize:12,color:"rgba(245,249,240,0.7)",fontFamily:"'DM Sans',sans-serif",letterSpacing:"0.06em"}}>🌿 Sustainably sourced · Free delivery over ₦30,000</span></div>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(245,249,240,0.97)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(58,125,68,0.1)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,40px)",height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:"clamp(18px,3vw,24px)",fontWeight:600,color:"#1A2E1C",letterSpacing:"0.02em",fontStyle:"italic"}}>{store.name}</span>
          <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",borderRadius:99,background:"rgba(58,125,68,0.08)",border:"1px solid rgba(58,125,68,0.12)",flex:1,maxWidth:280,margin:"0 24px"}}>
            <Search size={13} color={brand}/><input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Find plants…" style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,fontFamily:"'DM Sans',sans-serif",color:"#1A2E1C"}}/>
          </div>
          <button onClick={toggle} style={{background:brand,border:"none",borderRadius:99,padding:"9px 18px",cursor:"pointer",color:"#F5F9F0",fontFamily:"'DM Sans',sans-serif",fontSize:13,fontWeight:500,display:"flex",alignItems:"center",gap:6}}><ShoppingCart size={14}/>{count>0?`(${count})`:"Cart"}</button>
        </div>
      </header>
      <div style={{background:"linear-gradient(135deg,#E8F5E0,#D4EAC8)",padding:"clamp(56px,8vw,96px) clamp(16px,4vw,40px)",position:"relative",overflow:"hidden"}}>
        <div style={{position:"absolute",top:-40,right:-40,width:300,height:300,borderRadius:"50%",background:"rgba(58,125,68,0.06)",pointerEvents:"none"}}/>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:40,alignItems:"center"}}>
          <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.8}}>
            <p style={{fontSize:11,color:brand,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:16,fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>🌱 Freshly Arrived</p>
            <h1 style={{fontSize:"clamp(44px,7vw,80px)",fontWeight:300,color:"#1A2E1C",lineHeight:1.05,margin:"0 0 20px",letterSpacing:"-0.02em"}}>Bring<br/><em style={{fontStyle:"italic",color:brand}}>Nature</em><br/>Home.</h1>
            <p style={{fontSize:16,color:"rgba(26,46,28,0.6)",lineHeight:1.8,maxWidth:420,margin:"0 0 32px",fontFamily:"'DM Sans',sans-serif",fontWeight:300}}>{store.description||"Rare and beautiful plants delivered to your door. Each one sustainably grown with love."}</p>
            <a href="#products" style={{display:"inline-block",padding:"14px 32px",borderRadius:99,background:brand,color:"#F5F9F0",fontSize:14,fontWeight:500,textDecoration:"none",fontFamily:"'DM Sans',sans-serif",boxShadow:`0 8px 24px ${brand}35`}}>Shop Plants</a>
          </motion.div>
          {w>768&&products[0]&&<motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} transition={{duration:0.8,delay:0.2}} style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
            {products.slice(0,4).map((p,i)=><div key={p.id||i} style={{borderRadius:20,overflow:"hidden",marginTop:i%2===1?24:0,boxShadow:"0 4px 16px rgba(58,125,68,0.1)"}}>
              <img src={p.images?.[0]||"https://images.unsplash.com/photo-1485955900006-10f4d324d411?w=400&h=500&fit=crop"} alt={p.name} style={{width:"100%",aspectRatio:"3/4",objectFit:"cover"}}/>
            </div>)}
          </motion.div>}
        </div>
      </div>
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"16px clamp(16px,4vw,40px)",background:"#ECF5E4",borderBottom:"1px solid rgba(58,125,68,0.1)",overflowX:"auto"}}>
          <div style={{display:"flex",gap:8,maxWidth:1200,margin:"0 auto"}}>
            {categories.map(c=><button key={c} onClick={()=>onCategory?.(c)} style={{padding:"8px 20px",borderRadius:99,border:`1px solid ${category===c?brand:"rgba(58,125,68,0.2)"}`,background:category===c?brand:"transparent",color:category===c?"#F5F9F0":"rgba(26,46,28,0.6)",fontSize:13,fontWeight:400,cursor:"pointer",fontFamily:"'DM Sans',sans-serif",whiteSpace:"nowrap",transition:"all 0.2s"}}>{c}</button>)}
          </div>
        </div>
      )}
      <div id="products" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(40px,6vw,64px) clamp(16px,4vw,40px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"240px"},1fr))`,gap:20}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:20,background:"rgba(58,125,68,0.06)",aspectRatio:"3/4",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            ...
            const{add}=useCart(store.id,currency); const[added,setAdded]=useState(false);
            return(<div key={p.id} className="flora-card" style={{borderRadius:20,overflow:"hidden",background:"#fff",boxShadow:"0 2px 16px rgba(58,125,68,0.06)"}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{aspectRatio:"3/4",overflow:"hidden",background:"#E8F5E0"}}>
                  <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.6s"}}/>
                </div>
                <div style={{padding:"14px 16px 16px"}}>
                  <p style={{fontSize:12,color:brand,margin:"0 0 4px",fontFamily:"'DM Sans',sans-serif",fontWeight:500}}>{p.category||"Plants"}</p>
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: LUXE — Real Estate/Premium Property (Dark sophisticated)
// ══════════════════════════════════════════════════════════════════════════════
function LuxeTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  const brand=store.primaryColor||"#C0972E"; const currency=store.currency||"NGN";
  const{count,toggle}=useCart(store.id,currency); const w=useWindowSize();
  const[activeIdx,setActiveIdx]=useState(0);
  return(
    <div style={{minHeight:"100vh",background:"#0C0C0C",color:"#F2EDE4",fontFamily:"'Optima','Garamond','Times New Roman',serif"}}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(12,12,12,0.98)",backdropFilter:"blur(20px)",borderBottom:`1px solid rgba(192,151,46,0.1)`}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(20px,4vw,48px)",height:72,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:"clamp(14px,2vw,18px)",fontWeight:400,color:"rgba(242,237,228,0.6)",letterSpacing:"0.22em",textTransform:"uppercase"}}>ESTABLISHED</span>
          <span style={{fontSize:"clamp(16px,2.5vw,22px)",fontWeight:400,letterSpacing:"0.12em",textTransform:"uppercase",textAlign:"center"}}>{store.name}</span>
          <button onClick={toggle} style={{background:"none",border:"none",cursor:"pointer",display:"flex",alignItems:"center",gap:8,color:"rgba(242,237,228,0.6)",fontSize:12,letterSpacing:"0.1em",textTransform:"uppercase",fontFamily:"inherit"}}>
            <ShoppingCart size={14}/> {count>0?`(${count})`:"BAG"}
          </button>
        </div>
      </header>
      <div style={{position:"relative",minHeight:"90vh",display:"flex",alignItems:"center",overflow:"hidden"}}>
        <AnimatePresence mode="wait">
          {products[activeIdx]&&<motion.img key={activeIdx} initial={{opacity:0,scale:1.03}} animate={{opacity:1,scale:1}} exit={{opacity:0}} transition={{duration:1.2}}
            src={products[activeIdx].images?.[0]||"https://images.unsplash.com/photo-1560518883-ce09059eeffa?w=1600&h=900&fit=crop"} alt=""
            style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.35)"}}/>}
        </AnimatePresence>
        <div style={{position:"relative",zIndex:1,maxWidth:1280,margin:"0 auto",padding:"0 clamp(20px,4vw,48px)",width:"100%"}}>
          <motion.div initial={{opacity:0,y:24}} animate={{opacity:1,y:0}} transition={{duration:1,delay:0.3}}>
            <div style={{width:48,height:1,background:brand,marginBottom:24}}/>
            <p style={{fontSize:11,letterSpacing:"0.3em",color:brand,textTransform:"uppercase",marginBottom:16,fontWeight:400}}>Exclusive Collection</p>
            <h1 style={{fontSize:"clamp(44px,7vw,88px)",fontWeight:300,lineHeight:0.95,margin:"0 0 24px",letterSpacing:"-0.01em"}}>{store.name}<br/><em style={{fontStyle:"italic"}}>Collection</em></h1>
            <p style={{fontSize:15,color:"rgba(242,237,228,0.5)",lineHeight:1.9,maxWidth:440,margin:"0 0 40px",fontWeight:300}}>{store.description||"Exceptional properties and premium investments for discerning clients."}</p>
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"100%":"320px"},1fr))`,gap:24}}>
          {isLoading?Array.from({length:4}).map((_,i)=><div key={i} style={{borderRadius:4,background:"rgba(192,151,46,0.04)",aspectRatio:"4/3",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            ...
            return(<div key={p.id} style={{background:"#161616",border:`1px solid rgba(192,151,46,0.08)`}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{position:"relative",overflow:"hidden"}}>
                  <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"4/3",objectFit:"cover",display:"block",transition:"transform 0.6s"}}/>
                  <div style={{position:"absolute",top:14,right:14,background:brand,padding:"4px 12px"}}><span style={{fontSize:10,color:"#0C0C0C",fontWeight:600,letterSpacing:"0.08em",textTransform:"uppercase"}}>{p.category||"PREMIUM"}</span></div>
                </div>
                <div style={{padding:"20px"}}>
                  <p style={{fontSize:18,fontWeight:300,color:"#F2EDE4",margin:"0 0 8px",letterSpacing:"0.01em"}}>{p.name}</p>
                  <p style={{fontSize:12,color:"rgba(242,237,228,0.4)",margin:"0 0 16px",lineHeight:1.6}}>{(p.description||"Premium listing with exceptional features.").slice(0,80)}</p>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:22,fontWeight:300,color:brand}}>{fmtPrice(p.price,currency)}</span>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{padding:"10px 20px",border:`1px solid ${brand}`,background:"transparent",color:brand,fontSize:11,fontWeight:400,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.1em",textTransform:"uppercase",transition:"all 0.2s"}}>{added?"ENQUIRED":"ENQUIRE"}</button>
                  </div>
                </div>
              </Link>
            </div>);
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: MUSE — Creative Agency/Digital Products (Bold editorial)
// ══════════════════════════════════════════════════════════════════════════════
function MuseTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  const brand=store.primaryColor||"#FF6B2B"; const currency=store.currency||"NGN";
  const{count,toggle}=useCart(store.id,currency); const w=useWindowSize();
  const[mq,setMq]=useState(0);
  useEffect(()=>{const t=setInterval(()=>setMq(p=>p-0.5),16);return()=>clearInterval(t);},[]);
  const ticker=`${store.name.toUpperCase()} · CREATIVE · DESIGN · DIGITAL · `;
  return(
    <div style={{minHeight:"100vh",background:"#F4F0EB",fontFamily:"'Helvetica Neue','Arial',sans-serif",overflowX:"hidden"}}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"#F4F0EB",borderBottom:"2px solid #1A1A1A"}}>
        <div style={{maxWidth:1400,margin:"0 auto",padding:"0 clamp(16px,3vw,32px)",height:60,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:"clamp(16px,2.5vw,20px)",fontWeight:900,letterSpacing:"-0.04em",color:"#1A1A1A"}}>{store.name}</span>
          <div style={{overflow:"hidden",flex:1,margin:"0 24px"}}>
            <div style={{whiteSpace:"nowrap",transform:`translateX(${mq%((ticker.length)*8)}px)`,display:"flex"}}>
              {Array.from({length:6}).map((_,i)=><span key={i} style={{fontSize:11,fontWeight:700,color:"rgba(26,26,26,0.4)",letterSpacing:"0.12em",paddingRight:40}}>{ticker}</span>)}
            </div>
          </div>
          <button onClick={toggle} style={{background:"#1A1A1A",border:"none",padding:"8px 16px",color:"#F4F0EB",cursor:"pointer",fontSize:12,fontWeight:700,fontFamily:"inherit",letterSpacing:"0.06em",display:"flex",alignItems:"center",gap:6}}><ShoppingCart size={12}/>({count})</button>
        </div>
      </header>
      <div style={{padding:"clamp(48px,8vw,96px) clamp(16px,3vw,32px)",background:"#1A1A1A",overflow:"hidden"}}>
        <div style={{maxWidth:1400,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"1fr 1fr":"1fr",gap:32,alignItems:"center"}}>
          <div>
            <p style={{fontSize:11,color:brand,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:16,fontWeight:700}}>Creative Studio</p>
            <h1 style={{fontSize:"clamp(52px,9vw,108px)",fontWeight:900,color:"#F4F0EB",lineHeight:0.88,margin:"0 0 24px",letterSpacing:"-0.05em"}}>
              WE<br/>CREATE<br/><span style={{color:brand}}>ART.</span>
            </h1>
            <p style={{fontSize:15,color:"rgba(244,240,235,0.5)",lineHeight:1.7,maxWidth:420,margin:"0 0 32px"}}>{store.description||"Digital products, design assets, and creative tools for forward-thinking creators."}</p>
            <a href="#work" style={{display:"inline-block",padding:"14px 32px",background:brand,color:"#fff",fontSize:14,fontWeight:700,textDecoration:"none",letterSpacing:"0.04em"}}>BROWSE WORK →</a>
          </div>
          {w>768&&products[0]&&<div style={{position:"relative"}}>
            <div style={{position:"absolute",inset:-4,background:`${brand}20`,transform:"rotate(2deg)"}}/>
            <img src={products[0].images?.[0]||"https://images.unsplash.com/photo-1561070791-2526d30994b5?w=600&h=600&fit=crop"} alt="" style={{width:"100%",aspectRatio:"1",objectFit:"cover",position:"relative",zIndex:1}}/>
          </div>}
        </div>
      </div>
      <div id="work" style={{maxWidth:1400,margin:"0 auto",padding:"clamp(40px,6vw,64px) clamp(16px,3vw,32px)"}}>
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"280px"},1fr))`,gap:3,background:"#1A1A1A"}}>
          {isLoading?Array.from({length:6}).map((_,i)=><div key={i} style={{background:"#F4F0EB",aspectRatio:"1",animation:"pulse 1.5s infinite"}}/>):products.map((p,idx)=>{
            ...
            return(<div key={p.id} style={{background:"#F4F0EB",gridColumn:isBig&&w>768?"span 1":"span 1",position:"relative"}} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{position:"relative",aspectRatio:"1",overflow:"hidden"}}>
                  <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.5s",transform:hov?"scale(1.06)":"scale(1)"}}/>
                  <div style={{position:"absolute",inset:0,background:hov?"rgba(26,26,26,0.4)":"rgba(26,26,26,0)",transition:"background 0.3s"}}>
                    {hov&&<div style={{position:"absolute",bottom:16,left:16,right:16,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <span style={{fontSize:14,fontWeight:700,color:"#F4F0EB",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1,marginRight:12}}>{p.name}</span>
                      <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{background:added?"#1A1A1A":brand,border:"none",color:"#fff",padding:"6px 14px",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",whiteSpace:"nowrap",flexShrink:0}}>{added?"✓ GOT IT":"GET IT"}</button>
                    </div>}
                  </div>
                </div>
                {!hov&&<div style={{padding:"10px 12px 14px"}}>
                  <p style={{fontSize:13,fontWeight:700,color:"#1A1A1A",margin:"0 0 3px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.name}</p>
                  <p style={{fontSize:14,fontWeight:900,color:brand,margin:0}}>{fmtPrice(p.price,currency)}</p>
                </div>}
              </Link>
            </div>);
          })}
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: PEARL — Skincare/Beauty Premium (Cream, minimal, spa-like)
// ══════════════════════════════════════════════════════════════════════════════
function PearlTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  const brand=store.primaryColor||"#C8936C"; const currency=store.currency||"NGN";
  const{count,toggle}=useCart(store.id,currency); const w=useWindowSize();
  return(
    <div style={{minHeight:"100vh",background:"#FDFBF8",color:"#2C1A0E",fontFamily:"'EB Garamond','Georgia',serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=EB+Garamond:ital,wght@0,400;0,500;1,400&family=Mulish:wght@300;400;500;600&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      <div style={{background:"#2C1A0E",padding:"10px",textAlign:"center"}}><span style={{fontSize:12,color:"rgba(253,251,248,0.6)",fontFamily:"'Mulish',sans-serif",letterSpacing:"0.08em"}}>✨ Clean ingredients · Dermatologist tested · Free of parabens</span></div>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(253,251,248,0.97)",backdropFilter:"blur(20px)",borderBottom:"1px solid rgba(200,147,108,0.1)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,40px)",height:68,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:"clamp(18px,3vw,26px)",fontWeight:400,color:"#2C1A0E",letterSpacing:"0.06em",fontStyle:"italic"}}>{store.name}</span>
          <div style={{display:"flex",alignItems:"center",gap:16}}>
            <Search size={16} color="rgba(44,26,14,0.4)" style={{cursor:"pointer"}}/>
            <button onClick={toggle} style={{background:brand,border:"none",borderRadius:99,padding:"9px 20px",cursor:"pointer",color:"#FDFBF8",fontFamily:"'Mulish',sans-serif",fontSize:12,fontWeight:600,letterSpacing:"0.04em",display:"flex",alignItems:"center",gap:8}}><ShoppingCart size={13}/>{count>0?`(${count})`:"BAG"}</button>
          </div>
        </div>
      </header>
      <div style={{background:"linear-gradient(145deg,#F5EDE0,#EDE0CE)",padding:"clamp(64px,10vw,120px) clamp(16px,4vw,40px)",textAlign:"center"}}>
        <motion.div initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{duration:0.9}}>
          <p style={{fontSize:11,color:brand,letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:16,fontFamily:"'Mulish',sans-serif",fontWeight:500}}>Luxury Skincare</p>
          <h1 style={{fontSize:"clamp(44px,7vw,88px)",fontWeight:400,color:"#2C1A0E",lineHeight:1.05,margin:"0 0 20px",letterSpacing:"-0.01em"}}>
            Your Skin<br/><em style={{fontStyle:"italic",color:brand}}>Deserves</em><br/>The Best.
          </h1>
          <p style={{fontSize:16,color:"rgba(44,26,14,0.55)",lineHeight:1.9,maxWidth:520,margin:"0 auto 40px",fontFamily:"'Mulish',sans-serif",fontWeight:300}}>{store.description||"Scientifically formulated skincare that delivers visible results. Because your skin is worth it."}</p>
          <div style={{display:"flex",gap:12,justifyContent:"center",flexWrap:"wrap"}}>
            <a href="#products" style={{padding:"14px 36px",borderRadius:99,background:brand,color:"#FDFBF8",fontSize:13,fontWeight:600,textDecoration:"none",fontFamily:"'Mulish',sans-serif",boxShadow:`0 8px 24px ${brand}35`,letterSpacing:"0.04em"}}>Shop Collection</a>
            <a href="#ritual" style={{padding:"14px 36px",borderRadius:99,border:`1px solid rgba(200,147,108,0.3)`,color:brand,fontSize:13,fontWeight:500,textDecoration:"none",fontFamily:"'Mulish',sans-serif"}}>Our Ritual</a>
          </div>
        </motion.div>
      </div>
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"20px clamp(16px,4vw,40px)",background:"#FDFBF8",borderBottom:"1px solid rgba(200,147,108,0.08)",overflowX:"auto",textAlign:"center"}}>
          <div style={{display:"inline-flex",gap:8}}>
            {categories.map(c=><button key={c} onClick={()=>onCategory?.(c)} style={{padding:"8px 20px",borderRadius:99,border:`1px solid ${category===c?brand:"rgba(200,147,108,0.2)"}`,background:category===c?brand:"transparent",color:category===c?"#FDFBF8":"rgba(44,26,14,0.6)",fontSize:12,fontWeight:500,cursor:"pointer",fontFamily:"'Mulish',sans-serif",whiteSpace:"nowrap",transition:"all 0.2s"}}>{c}</button>)}
          </div>
        </div>
      )}
      <div id="products" style={{maxWidth:1200,margin:"0 auto",padding:"clamp(48px,6vw,72px) clamp(16px,4vw,40px)"}}>
        <div style={{textAlign:"center",marginBottom:48}}>
          <h2 style={{fontSize:"clamp(28px,4vw,44px)",fontWeight:400,color:"#2C1A0E",margin:"0 0 8px"}}>The Collection</h2>
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"44vw":"240px"},1fr))`,gap:24}}>
          {isLoading?Array.from({length:8}).map((_,i)=><div key={i} style={{borderRadius:20,background:"rgba(200,147,108,0.06)",aspectRatio:"3/4",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            ...
            return(<div key={p.id} style={{borderRadius:20,overflow:"hidden",background:"#fff",boxShadow:"0 4px 24px rgba(200,147,108,0.06)"}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{position:"relative",aspectRatio:"3/4",overflow:"hidden",background:"#F5EDE0"}}>
                  <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.5s"}}/>
                  {disc>0&&<div style={{position:"absolute",top:14,right:14,background:brand,borderRadius:99,padding:"3px 10px"}}><span style={{fontSize:10,color:"#fff",fontWeight:600,fontFamily:"'Mulish',sans-serif"}}>-{disc}%</span></div>}
                </div>
                <div style={{padding:"16px"}}>
                  <p style={{fontSize:12,color:brand,margin:"0 0 4px",fontFamily:"'Mulish',sans-serif",fontWeight:500}}>{p.category||"Skincare"}</p>
                  <p style={{fontSize:16,fontWeight:400,color:"#2C1A0E",margin:"0 0 12px"}}>{p.name}</p>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <div><span style={{fontSize:17,fontWeight:500,color:brand}}>{fmtPrice(p.price,currency)}</span>{p.comparePrice&&<span style={{fontSize:12,color:"#ccc",textDecoration:"line-through",marginLeft:8,fontFamily:"'Mulish',sans-serif"}}>{fmtPrice(p.comparePrice,currency)}</span>}</div>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{padding:"7px 14px",borderRadius:99,background:added?brand:"rgba(200,147,108,0.1)",border:`1px solid ${added?brand:"rgba(200,147,108,0.2)"}`,color:added?"#FDFBF8":brand,fontSize:11,fontWeight:600,cursor:"pointer",fontFamily:"'Mulish',sans-serif",transition:"all 0.2s"}}>{added?"✓ Added":"Add"}</button>
                  </div>
                </div>
              </Link>
            </div>);
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: CHROME — Auto/Car Dealership (Metallic, bold, masculine)
// ══════════════════════════════════════════════════════════════════════════════
function ChromeTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  const brand=store.primaryColor||"#E8A020"; const currency=store.currency||"NGN";
  const{count,toggle}=useCart(store.id,currency); const w=useWindowSize();
  return(
    <div style={{minHeight:"100vh",background:"#0F0F0F",color:"#F0F0F0",fontFamily:"'Oswald','Impact',sans-serif"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Oswald:wght@300;400;600;700&display=swap');@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.4}}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(15,15,15,0.98)",borderBottom:`3px solid ${brand}`}}>
        <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,3vw,32px)",height:64,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
          <span style={{fontSize:"clamp(18px,3vw,26px)",fontWeight:700,color:"#F0F0F0",letterSpacing:"0.06em",textTransform:"uppercase"}}>{store.name}</span>
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"8px 14px",border:`1px solid rgba(232,160,32,0.2)`,background:"rgba(232,160,32,0.04)"}}>
              <Search size={13} color={brand}/><input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Search vehicles…" style={{background:"transparent",border:"none",outline:"none",fontSize:13,color:"#F0F0F0",fontFamily:"inherit",width:160}}/>
            </div>
            <button onClick={toggle} style={{background:brand,border:"none",padding:"10px 20px",color:"#0F0F0F",cursor:"pointer",fontSize:13,fontWeight:700,fontFamily:"inherit",letterSpacing:"0.06em",display:"flex",alignItems:"center",gap:8}}><ShoppingCart size={13}/>{count>0?`[${count}]`:""} ENQUIRE</button>
          </div>
        </div>
      </header>
      <div style={{position:"relative",height:"85vh",overflow:"hidden"}}>
        {products[0]&&<img src={products[0].images?.[0]||"https://images.unsplash.com/photo-1492144534655-ae79c964c9d7?w=1600&h=900&fit=crop"} alt="" style={{width:"100%",height:"100%",objectFit:"cover",filter:"brightness(0.4)"}}/>}
        <div style={{position:"absolute",inset:0,background:"linear-gradient(to right,rgba(15,15,15,0.85) 40%,transparent)"}}/>
        <div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",padding:"0 clamp(20px,5vw,80px)"}}>
          <motion.div initial={{opacity:0,x:-24}} animate={{opacity:1,x:0}} transition={{duration:0.8}}>
            <p style={{fontSize:11,color:brand,letterSpacing:"0.3em",textTransform:"uppercase",marginBottom:16,fontWeight:400}}>Premium Inventory</p>
            <h1 style={{fontSize:"clamp(48px,9vw,108px)",fontWeight:700,lineHeight:0.88,margin:"0 0 24px",textTransform:"uppercase",letterSpacing:"-0.02em"}}>{store.name}</h1>
            <p style={{fontSize:15,color:"rgba(240,240,240,0.5)",lineHeight:1.7,maxWidth:440,margin:"0 0 36px",fontFamily:"'Helvetica Neue',sans-serif",fontWeight:300}}>{store.description||"Hand-selected premium vehicles. Every car inspected, verified, and ready to drive."}</p>
            <a href="#inventory" style={{display:"inline-block",padding:"14px 40px",background:brand,color:"#0F0F0F",fontSize:14,fontWeight:700,textDecoration:"none",letterSpacing:"0.1em",textTransform:"uppercase"}}>VIEW INVENTORY</a>
          </motion.div>
        </div>
      </div>
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{borderBottom:`1px solid rgba(232,160,32,0.1)`,overflowX:"auto",background:"#161616"}}>
          <div style={{maxWidth:1280,margin:"0 auto",padding:"0 clamp(16px,3vw,32px)",display:"flex",gap:0}}>
            {categories.map(c=><button key={c} onClick={()=>onCategory?.(c)} style={{padding:"14px 20px",border:"none",borderBottom:`3px solid ${category===c?brand:"transparent"}`,background:"transparent",color:category===c?brand:"rgba(240,240,240,0.35)",fontSize:12,fontWeight:700,cursor:"pointer",letterSpacing:"0.12em",textTransform:"uppercase",fontFamily:"inherit",whiteSpace:"nowrap"}}>{c}</button>)}
          </div>
        </div>
      )}
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"100%":"320px"},1fr))`,gap:16}}>
          {isLoading?Array.from({length:4}).map((_,i)=><div key={i} style={{background:"#1A1A1A",aspectRatio:"16/9",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            ...
            return(<div key={p.id} style={{background:"#1A1A1A",border:`1px solid rgba(232,160,32,0.08)`,overflow:"hidden"}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{position:"relative",overflow:"hidden"}}>
                  <img src={img} alt={p.name} style={{width:"100%",aspectRatio:"16/9",objectFit:"cover",display:"block",transition:"transform 0.5s"}}/>
                  <div style={{position:"absolute",top:12,left:12,background:brand,padding:"3px 10px"}}><span style={{fontSize:10,color:"#0F0F0F",fontWeight:700,letterSpacing:"0.08em"}}>{p.category||"AVAILABLE"}</span></div>
                </div>
                <div style={{padding:"18px"}}>
                  <p style={{fontSize:18,fontWeight:600,color:"#F0F0F0",margin:"0 0 6px",letterSpacing:"0.02em",textTransform:"uppercase"}}>{p.name}</p>
                  <p style={{fontSize:12,color:"rgba(240,240,240,0.4)",margin:"0 0 14px",lineHeight:1.5,fontFamily:"'Helvetica Neue',sans-serif"}}>{(p.description||"Premium condition. Full service history.").slice(0,60)}</p>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                    <span style={{fontSize:22,fontWeight:700,color:brand}}>{fmtPrice(p.price,currency)}</span>
                    <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{padding:"10px 20px",background:added?"#2A2A2A":brand,border:`1px solid ${brand}`,color:added?"#F0F0F0":"#0F0F0F",fontSize:11,fontWeight:700,cursor:"pointer",fontFamily:"inherit",letterSpacing:"0.08em",textTransform:"uppercase"}}>{added?"SAVED ✓":"GET QUOTE"}</button>
                  </div>
                </div>
              </Link>
            </div>);
...
// ══════════════════════════════════════════════════════════════════════════════
//  TEMPLATE: BOUND — Bookstore (Literary, warm, intellectual)
// ══════════════════════════════════════════════════════════════════════════════
function BoundTemplate({ store, products=[], search, onSearch, category, onCategory, categories=[], isLoading }: TemplateProps) {
  const brand=store.primaryColor||"#8B3A3A"; const currency=store.currency||"NGN";
  const{count,toggle}=useCart(store.id,currency); const w=useWindowSize();
  return(
    <div style={{minHeight:"100vh",background:"#F9F4EE",color:"#1A1008",fontFamily:"'Bookerly','Georgia','Times New Roman',serif"}}>
      <style>{`@keyframes pulse{0%,100%{opacity:1}50%{opacity:0.5}}`}</style>
      <header style={{position:"sticky",top:0,zIndex:50,background:"rgba(249,244,238,0.97)",backdropFilter:"blur(16px)",borderBottom:"1px solid rgba(139,58,58,0.1)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",padding:"0 clamp(16px,4vw,40px)",height:64,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
          <span style={{fontSize:"clamp(18px,3vw,26px)",fontWeight:700,color:"#1A1008",letterSpacing:"-0.01em"}}>{store.name}</span>
          <div style={{display:"flex",flex:1,maxWidth:300,alignItems:"center",gap:8,padding:"8px 14px",borderRadius:4,border:"1px solid rgba(139,58,58,0.12)",background:"rgba(139,58,58,0.03)"}}>
            <Search size={13} color={brand}/><input value={search||""} onChange={e=>onSearch?.(e.target.value)} placeholder="Search books…" style={{flex:1,background:"transparent",border:"none",outline:"none",fontSize:13,color:"#1A1008",fontFamily:"inherit"}}/>
          </div>
          <button onClick={toggle} style={{background:brand,border:"none",borderRadius:4,padding:"9px 18px",cursor:"pointer",color:"#F9F4EE",fontFamily:"'Helvetica Neue',sans-serif",fontSize:12,fontWeight:600,display:"flex",alignItems:"center",gap:6}}><ShoppingCart size={13}/>{count>0?`(${count})`:"Cart"}</button>
        </div>
      </header>
      <div style={{background:"linear-gradient(135deg,#EDE3D5,#DDD0BB)",padding:"clamp(48px,8vw,96px) clamp(16px,4vw,40px)"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:w>768?"2fr 1fr":"1fr",gap:40,alignItems:"center"}}>
          <div>
            <p style={{fontSize:11,color:brand,letterSpacing:"0.2em",textTransform:"uppercase",marginBottom:16,fontFamily:"'Helvetica Neue',sans-serif",fontWeight:500}}>Est. {new Date().getFullYear()} · Curated Books</p>
            <h1 style={{fontSize:"clamp(44px,7vw,80px)",fontWeight:700,color:"#1A1008",lineHeight:1.0,margin:"0 0 20px",letterSpacing:"-0.02em"}}>
              Stories that<br/><em style={{fontStyle:"italic",color:brand}}>Change</em><br/>Everything.
            </h1>
            <p style={{fontSize:16,color:"rgba(26,16,8,0.55)",lineHeight:1.9,maxWidth:480,margin:"0 0 32px",fontFamily:"'Helvetica Neue',sans-serif",fontWeight:300}}>{store.description||"Carefully curated books across every genre. Discover your next great read."}</p>
      ...
      {categories.filter(c=>c!=="All").length>0&&(
        <div style={{padding:"16px clamp(16px,4vw,40px)",background:"#F0E8DA",borderBottom:"1px solid rgba(139,58,58,0.1)",overflowX:"auto"}}>
          <div style={{display:"flex",gap:8,maxWidth:1200,margin:"0 auto"}}>
            {categories.map(c=><button key={c} onClick={()=>onCategory?.(c)} style={{padding:"8px 18px",borderRadius:4,border:`1px solid ${category===c?brand:"rgba(139,58,58,0.15)"}`,background:category===c?brand:"transparent",color:category===c?"#F9F4EE":"rgba(26,16,8,0.6)",fontSize:13,fontWeight:500,cursor:"pointer",fontFamily:"'Helvetica Neue',sans-serif",whiteSpace:"nowrap",transition:"all 0.2s"}}>{c}</button>)}
          </div>
        </div>
      )}
        ...
        <div style={{display:"grid",gridTemplateColumns:`repeat(auto-fill,minmax(${w<640?"140px":"180px"},1fr))`,gap:24}}>
          {isLoading?Array.from({length:10}).map((_,i)=><div key={i} style={{borderRadius:4,background:"rgba(139,58,58,0.06)",aspectRatio:"2/3",animation:"pulse 1.5s infinite"}}/>):products.map(p=>{
            ...
            return(<div key={p.id} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)} style={{cursor:"pointer"}}>
              <Link href={`/store/${store.slug}/product/${p.id}`} style={{textDecoration:"none",display:"block"}}>
                <div style={{aspectRatio:"2/3",borderRadius:4,overflow:"hidden",boxShadow:hov?"6px 6px 20px rgba(26,16,8,0.2)":"2px 4px 10px rgba(26,16,8,0.1)",transition:"box-shadow 0.3s",marginBottom:10}}>
                  <img src={img} alt={p.name} style={{width:"100%",height:"100%",objectFit:"cover",transition:"transform 0.4s",transform:hov?"scale(1.04)":"scale(1)"}}/>
                </div>
                <p style={{fontSize:13,fontWeight:600,color:"#1A1008",margin:"0 0 3px",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",lineHeight:1.3}}>{p.name}</p>
                <p style={{fontSize:12,color:"rgba(26,16,8,0.5)",margin:"0 0 6px",fontFamily:"'Helvetica Neue',sans-serif"}}>{p.category||"General"}</p>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <span style={{fontSize:14,fontWeight:700,color:brand}}>{fmtPrice(p.price,currency)}</span>
                  <button onClick={e=>{e.preventDefault();e.stopPropagation();add(p);setAdded(true);setTimeout(()=>setAdded(false),1500)}} style={{background:added?brand:"rgba(139,58,58,0.1)",border:"none",color:added?"#F9F4EE":brand,width:28,height:28,borderRadius:"50%",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",transition:"all 0.2s"}}>{added?<Check size={12}/>:<Plus size={12}/>}</button>
                </div>
              </Link>
            </div>);
          })}
