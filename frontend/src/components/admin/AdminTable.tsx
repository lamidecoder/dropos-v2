...
export function StatGrid({ children, cols=4 }: { children:ReactNode; cols?:number }) {
  return (
    <>
      <div style={{ display:"grid", gridTemplateColumns:`repeat(${cols},1fr)`, gap:12, marginBottom:20 }} className="adm-stat-grid">
        {children}
      </div>
      <style>{`@media(max-width:900px){ .adm-stat-grid{ grid-template-columns:1fr 1fr!important; } } @media(max-width:480px){ .adm-stat-grid{ gap:8px!important; } }`}</style>
    </>
  );
}
...
export function FilterBar({ children }: { children:ReactNode }) {
  return (
    <>
      ...
      <style>{`@media(max-width:640px){ .adm-filter>div,.adm-filter>select,.adm-filter>input{ width:100%!important; flex:none!important; } }`}</style>
    </>
  );
}
...
export function Pagination({ page, pages, total, onPage }: { page:number; pages:number; total:number; onPage:(p:number)=>void }) {
  ...
  const btn = (active: boolean): any => ({ minWidth:32, height:32, padding:"0 10px", borderRadius:8, border:`1px solid ${active?"rgba(107,53,232,0.5)":"rgba(255,255,255,0.07)"}`, background:active?"rgba(107,53,232,0.15)":"transparent", color:active?"#A78BFA":"rgba(255,255,255,0.4)", cursor:"pointer", fontSize:12, fontWeight:active?700:400 });
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginTop:14, flexWrap:"wrap", gap:10 }}>
      <span style={{ fontSize:12, color:"rgba(255,255,255,0.3)" }}>{(total||0).toLocaleString()} total</span>
      <div style={{ display:"flex", gap:4, flexWrap:"wrap" }}>
        <button onClick={() => onPage(page-1)} disabled={page===1} style={{ ...btn(false), opacity:page===1?0.3:1 }}>←</button>
        {range.map(p => <button key={p} onClick={() => onPage(p)} style={btn(p===page)}>{p}</button>)}
        <button onClick={() => onPage(page+1)} disabled={page>=pages} style={{ ...btn(false), opacity:page>=pages?0.3:1 }}>→</button>
      </div>
    </div>
  );
}
...
export function DataTable({ cols, rows, loading, empty="No records found" }: {
  ...
  return (
    <div style={{ borderRadius:16, border:`1px solid ${t.border}`, overflow:"hidden", overflowX:"auto" }}>
      ...
      <style>{`@media(max-width:768px){ .adm-col-hide-md{display:none!important;} } @media(max-width:540px){ .adm-col-hide-sm{display:none!important;} }`}</style>
    </div>
  );
}
