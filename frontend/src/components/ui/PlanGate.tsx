    ...
    return (
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div style={{ flex: 1 }}>
          ...
          <p style={{ fontSize: 12, color: T.muted, lineHeight: 1.5 }}>{copy.desc}</p>
        </div>
        ...
        <button onClick={() => setDismissed(true)} style={{ color: T.faint, background: "none", border: "none", cursor: "pointer" }}><X size={14} /></button>
      </motion.div>
    );
  }
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4, fontSize: 11 }}>
        <span style={{ color: T.muted }}>{label || feature}</span>
        <span style={{ color: isHigh ? color : T.muted, fontWeight: isHigh ? 700 : 400 }}>
          {current} / {max}
        </span>
      </div>
