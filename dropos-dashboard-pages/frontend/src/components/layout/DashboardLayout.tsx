...
// Brand: Plus Jakarta Sans + Syncopate · Void dark · Violet system
...
import {
  ...
  Percent, Star, Shield, Sun, Moon, Search, Command,
...
const T = {
  dark: {
    ...
    navHover: "rgba(255,255,255,0.04)", headerBg: "rgba(6,4,13,0.85)",
  },
  light: {
    ...
    navHover: "rgba(15,5,32,0.04)", headerBg: "rgba(244,242,255,0.9)",
  },
};

const V = {
  v900: "#1A0D3D", v700: "#3D1C8A", v600: "#5428C8",
  v500: "#6B35E8", v400: "#8B5CF6", v300: "#A78BFA", v200: "#C4B5FD",
...
// ── NAV ITEM ──────────────────────────────────────────────────
function NavItem({ item, isActive, t }: { item: any; isActive: boolean; t: typeof T.dark }) {
  const Icon = item.icon;
  const isKIRO = item.ai;
  return (
    <Link href={item.href} style={{ display: "block", marginBottom: 1 }}>
      <div style={{
        position: "relative",
        display: "flex", alignItems: "center", gap: 10,
        padding: "8px 10px", borderRadius: 11,
        border: `1px solid ${isActive ? (isKIRO ? "rgba(107,53,232,0.25)" : "rgba(107,53,232,0.15)") : "transparent"}`,
        background: isActive ? (isKIRO ? "rgba(107,53,232,0.12)" : "rgba(107,53,232,0.08)") : "transparent",
        cursor: "pointer",
        transition: "all 0.15s",
      }}>
        {isActive && (
          <div style={{ position: "absolute", left: 0, top: "50%", transform: "translateY(-50%)", width: 3, height: 18, borderRadius: 99, background: isKIRO ? V.v300 : V.v500 }} />
        )}
        <div style={{
          width: 28, height: 28, borderRadius: 8,
          display: "flex", alignItems: "center", justifyContent: "center",
          flexShrink: 0,
          background: isActive ? (isKIRO ? "rgba(107,53,232,0.25)" : "rgba(107,53,232,0.18)") : t.navHover.replace("0.04", "0.04"),
        }}>
          <Icon size={13} color={isActive ? (isKIRO ? V.v300 : V.v400) : t.textMuted} />
        </div>
        <span style={{
          fontSize: 12.5, flex: 1,
          color: isActive ? (isKIRO ? V.v200 : "#fff") : t.textMuted,
        ...
        {item.ai && (
          <span style={{ fontFamily: "'Syncopate', sans-serif", fontSize: 7.5, fontWeight: 700, letterSpacing: "0.08em", padding: "2px 6px", borderRadius: 99, background: "rgba(107,53,232,0.2)", color: V.v300, border: "1px solid rgba(107,53,232,0.3)", flexShrink: 0 }}>AI</span>
        )}
        {item.badge && !item.ai && (
          <span style={{ fontFamily: "'Syncopate', sans-serif", fontSize: 7.5, fontWeight: 700, letterSpacing: "0.06em", padding: "2px 6px", borderRadius: 99, background: "rgba(245,158,11,0.12)", color: "#d97706", border: "1px solid rgba(245,158,11,0.2)", flexShrink: 0 }}>{item.badge}</span>
        )}
      </div>
    </Link>
  );
...
// ── NAV GROUP ─────────────────────────────────────────────────
function NavGroup({ group, pathname, t }: { group: any; pathname: string; t: typeof T.dark }) {
  const isActive = (item: any) =>
    item.exact ? pathname === item.href : pathname === item.href || (pathname.startsWith(item.href) && item.href !== "/dashboard" && item.href !== "/admin");
  ...
  if (group.alwaysOpen) return (
    <div style={{ marginBottom: 16 }}>
      {group.items.map((item: any) => <NavItem key={item.href} item={item} isActive={isActive(item)} t={t} />)}
    </div>
  );
  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <button onClick={() => setOpen(!open)} style={{
        ...
        padding: "6px 12px 4px", borderRadius: 8, border: "none",
        background: "transparent", cursor: "pointer",
        color: hasActive ? t.textMuted : t.textFaint,
        marginBottom: 2,
      }}>
        <span style={{ fontFamily: "'Syncopate', sans-serif", fontSize: 8.5, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", flex: 1, textAlign: "left" }}>
          {group.label}
        </span>
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  const planColor = plan === "PRO" ? V.v300 : plan === "GROWTH" ? V.v400 : "rgba(255,255,255,0.4)";
        ...
        <ConfirmContext.Provider value={{ confirm }}>
          ...
          <style>{`
            ...
            *, *::before, *::after { box-sizing: border-box; }
            ...
            @keyframes kiro-glow {
              0%, 100% { box-shadow: 0 4px 18px rgba(107,53,232,0.35); }
              50%       { box-shadow: 0 4px 28px rgba(107,53,232,0.65); }
            }
          ...
          <div style={{ display: "flex", height: "100vh", width: "100vw", overflow: "hidden", background: t.bg, color: t.text, fontFamily: "'Plus Jakarta Sans', sans-serif" }}>
            ...
            {/* ── SIDEBAR ── */}
            <aside style={{ width: 236, minWidth: 236, height: "100%", display: "flex", flexDirection: "column", background: theme === "dark" ? "#0D0918" : "#fff", borderRight: `1px solid ${t.border}`, flexShrink: 0, zIndex: 10 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "20px 16px", flexShrink: 0, borderBottom: `1px solid ${t.border}`, minHeight: 64 }}>
                ...
                <div style={{ flex: 1 }}>
                  <div>
                    <span style={{ fontWeight: 700, fontSize: 16, letterSpacing: "-0.02em", color: t.text }}>Drop</span>
                    <span style={{ fontFamily: "'Syncopate', sans-serif", fontWeight: 700, fontSize: 16, color: V.v400, letterSpacing: "-0.01em" }}>OS</span>
                  </div>
              ...
              <div className="hide-scroll" style={{ flex: 1, overflowY: "auto", padding: "12px 8px" }}>
                {nav.map(group => <NavGroup key={group.id} group={group} pathname={pathname} t={t} />)}
              </div>
              ...
              <div style={{ flexShrink: 0, padding: 8, borderTop: `1px solid ${t.border}` }}>
                ...
                {/* User */}
                <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 10px", borderRadius: 12, cursor: "pointer" }} onClick={handleLogout}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: t.text, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", letterSpacing: "-0.01em" }}>{user?.name || "User"}</div>
            ...
            <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflow: "hidden" }}>
              ...
              {/* Header */}
              <header style={{ height: 60, flexShrink: 0, display: "flex", alignItems: "center", gap: 12, padding: "0 24px", background: t.headerBg, backdropFilter: "blur(24px)", borderBottom: `1px solid ${t.border}`, zIndex: 20 }}>
                {/* Search */}
                <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", borderRadius: 12, flex: 1, maxWidth: 300, cursor: "pointer", background: theme === "dark" ? "rgba(255,255,255,0.04)" : "rgba(15,5,32,0.04)", border: `1px solid ${t.border}` }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                  ...
                  <Link href="/dashboard/kai">
                    <div className="kiro-pulse" style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 16px", borderRadius: 12, cursor: "pointer", background: `linear-gradient(135deg, ${V.v500}, ${V.v700})`, color: "#fff" }}>
              ...
              {/* Page */}
              <main className="thin-scroll" style={{ flex: 1, overflowY: "auto", padding: 28 }}>
                <AnimatePresence mode="wait">
                  <motion.div key={pathname} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.18 }}>
                    {children}
                  </motion.div>
                </AnimatePresence>
              </main>
            ...
            <AnimatePresence>
              {toasts.map(toast => (
                <motion.div key={toast.id} initial={{ opacity: 0, y: 16, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.95 }} transition={{ type: "spring", stiffness: 400, damping: 30 }}
                  style={{ pointerEvents: "auto", display: "flex", alignItems: "center", gap: 10, padding: "12px 16px", borderRadius: 14, boxShadow: "0 8px 32px rgba(0,0,0,0.2)", minWidth: 260, background: theme === "dark" ? "#181230" : "#fff", border: `1px solid ${t.border}` }}>
