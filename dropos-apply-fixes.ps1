# dropos-apply-fixes.ps1
# Run from your project root:
# powershell -ExecutionPolicy Bypass -File dropos-apply-fixes.ps1

$ROOT = "C:\Users\USER\Downloads\dropos-v2-fixed (2)\dropos-v2\frontend\src"

Write-Host ""
Write-Host " DropOS — Applying fixed files" -ForegroundColor Cyan
...
# ── 2. features/page.tsx ──────────────────────────────────────
$files["app/(marketing)/features/page.tsx"] = @'
"use client";
import { motion } from "framer-motion";
import Link from "next/link";
    ...
    <div className="min-h-screen" style={{ background: "var(--bg-base)" }}>
      <section className="pt-40 pb-24 px-6 text-center relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="relative z-10 max-w-3xl mx-auto">
          ...
          <h1 className="text-5xl md:text-6xl font-black tracking-tight mb-6" style={{ color: "var(--text-primary)" }}>
            Every tool your store needs.<br /><span style={{ color: "var(--violet-500)" }}>Run by AI.</span>
          </h1>
...
const PLANS = [
  {
    name: "Free", price: "N0", period: "forever",
    desc: "Start selling today. No credit card required.",
    color: "var(--text-secondary)", accent: "var(--border)", cta: "Start free",
    href: "/auth/register", featured: false,
    features: ["1 store","Up to 10 products","KIRO basic chat","Paystack checkout","2% transaction fee","Community support"],
  },
  {
    name: "Growth", price: "N9,500", period: "/month",
    desc: "For serious sellers ready to scale.",
    color: "var(--violet-400)", accent: "var(--violet-500)", cta: "Start Growth",
    href: "/auth/register?plan=growth", featured: true,
  ...
  {
    name: "Pro", price: "N25,000", period: "/month",
    desc: "For high-volume stores and agencies.",
    color: "#C026D3", accent: "#C026D3", cta: "Start Pro",
    href: "/auth/register?plan=pro", featured: false,
    features: ["Unlimited stores","Unlimited products","KIRO extended thinking","KIRO vision","Custom domain per store","Affiliate programme","API access","0.5% transaction fee","Dedicated support"],
  },
];
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button onClick={() => save.mutate({ platformFeePercent: fee, maintenanceMode: maintenance, allowRegistration: allowReg })}
      ...
      <div className={`rounded-2xl border p-6 ${card}`}>
        <div className="flex items-center gap-2 mb-4"><DollarSign size={16} className="text-violet-500" /><h3 className={`font-bold ${tx}`}>Platform Fee</h3></div>
        <div className="flex items-center justify-between">
      ...
      <div className={`rounded-2xl border p-6 ${card}`}>
        <div className="flex items-center gap-2 mb-4"><Package size={16} className="text-violet-500" /><h3 className={`font-bold ${tx}`}>Subscription Plans</h3></div>
      ...
      <div className={`rounded-2xl border p-6 ${card}`}>
        <div className="flex items-center gap-2 mb-4"><Users size={16} className="text-violet-500" /><h3 className={`font-bold ${tx}`}>User Registration</h3></div>
        <div className="flex items-center justify-between">
      ...
      <div className={`rounded-2xl border p-6 ${card}`}>
        <div className="flex items-center gap-2 mb-4"><Shield size={16} className="text-violet-500" /><h3 className={`font-bold ${tx}`}>Maintenance Mode</h3></div>
        <div className="flex items-center justify-between">
          <div>
        ...
        <div className="rounded-3xl p-10" style={{ background: "var(--bg-card)", border: "1px solid var(--border)" }}>
          ...
          {status === "error" && (
            <div>
              ...
              <Link href="/auth/login" className="inline-block px-6 py-3 rounded-xl font-bold text-sm text-white" style={{ background: "var(--violet-500)" }}>Back to login</Link>
            </div>
          )}
        </div>
      </div>
...
$files["components/store/AbandonedCartTracker.tsx"] = @'
"use client";
...
export default function AbandonedCartTracker({ store, exitDiscount = 10, idleMinutes = 30 }: AbandonedCartTrackerProps) {
  ...
  const handleClaim  = () => { dismissPopup(); window.location.href = "/cart"; };
  ...
  return (
    <div className="fixed inset-0 z-[9998] flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-sm rounded-3xl p-8 text-center" style={{ background: "var(--bg-card)", border: "1px solid var(--border)", zIndex: 9999 }}>
        ...
        <div className="space-y-3">
          <button onClick={handleClaim} className="w-full py-3.5 rounded-2xl font-bold text-sm text-white flex items-center justify-center gap-2 transition-all hover:opacity-90" style={{ background: "var(--violet-500)" }}>
...
$files["components/store/templates/TemplateRenderer.tsx"] = @'
"use client";
...
foreach ($rel in $files.Keys) {
  ...
  try {
    [System.IO.File]::WriteAllText($fullPath, $files[$rel], [System.Text.Encoding]::UTF8)
...
Write-Host ""
Write-Host "Written: $written  Errors: $errors" -ForegroundColor Cyan
Write-Host ""

# ── GIT PUSH ──────────────────────────────────────────────────
Set-Location "C:\Users\USER\Downloads\dropos-v2-fixed (2)\dropos-v2"

git add .
$status = git status --short
if ($status) {
  git commit -m "fix: replace all broken JSX pages with clean versions"
  git push origin main
  Write-Host ""
  Write-Host "Pushed. Watch Vercel now." -ForegroundColor Green
} else {
  Write-Host "Nothing changed — files may already be in git." -ForegroundColor Yellow
  Write-Host "Force a redeploy from vercel.com" -ForegroundColor Yellow
}
