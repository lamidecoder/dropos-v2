            ...
            <motion.div key="form" exit={{ opacity: 0, y: -16 }}>
              ...
              <div style={{ maxWidth: "620px", margin: "0 auto", textAlign: "center", padding: "72px 24px 48px" }}>
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <span><strong>{count.toLocaleString()}</strong> sellers already joined</span>
                </motion.div>
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }}
                  style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "14px", marginBottom: "36px" }}>
                  <div style={{ display: "flex" }}>
                    {["A","T","F","C","O"].map((l,i) => (
                      <div key={i} style={{ width: "36px", height: "36px", borderRadius: "50%", border: "2px solid #04030d", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: 700, color: "#fff", marginLeft: i === 0 ? 0 : "-10px", background: ["linear-gradient(135deg,#7c3aed,#5b21b6)","linear-gradient(135deg,#5b21b6,#4c1d95)","linear-gradient(135deg,#1d4ed8,#1e40af)","linear-gradient(135deg,#0f766e,#0d9488)","linear-gradient(135deg,#b45309,#92400e)"][i] }}>
                        {l}
                      </div>
                    ))}
                    <div style={{ width: "36px", height: "36px", borderRadius: "50%", border: "2px solid #04030d", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 600, color: "rgba(255,255,255,0.5)", marginLeft: "-10px", background: "rgba(255,255,255,0.08)" }}>+</div>
                  </div>
                  <div style={{ textAlign: "left" }}>
                    <div style={{ fontSize: "14px", color: "rgba(255,255,255,0.6)" }}><strong style={{ color: "#fff" }}>{count.toLocaleString()} sellers</strong> waiting</div>
                    <div style={{ color: "#fbbf24", fontSize: "13px", letterSpacing: "1px" }}>★★★★★</div>
                  </div>
                </motion.div>

                {/* Scarcity bar */}
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
                  style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.07)", borderRadius: "14px", padding: "16px 20px", marginBottom: "24px", display: "flex", alignItems: "center", gap: "14px" }}>
                  <span style={{ fontSize: "18px", flexShrink: 0 }}>🔥</span>
                  <div style={{ flex: 1, height: "6px", borderRadius: "3px", background: "rgba(255,255,255,0.08)", overflow: "hidden" }}>
                    <motion.div style={{ height: "100%", borderRadius: "3px", background: "linear-gradient(90deg,#7c3aed,#a78bfa)" }}
                      initial={{ width: "0%" }} animate={{ width: `${fillPct}%` }} transition={{ duration: 1.8, ease: [0.22,1,0.36,1] }} />
                  </div>
                  <div style={{ flexShrink: 0, textAlign: "right" }}>
                    <div style={{ fontSize: "18px", fontWeight: 800, color: "#fff", letterSpacing: "-0.5px", lineHeight: 1 }}>{spotsLeft}</div>
                    <div style={{ fontSize: "10px", color: "rgba(255,255,255,0.35)", textTransform: "uppercase", letterSpacing: "0.05em", marginTop: "1px" }}>spots left</div>
                  </div>
                </motion.div>
                ...
                <motion.div id="form" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
                  ...
                  <motion.button onClick={submit} disabled={status === "loading"}
                    ...
                    whileTap={{ scale: 0.98 }}>
                    {status === "loading" ? "Securing your spot..." : "Claim My Early Access →"}
                  </motion.button>
