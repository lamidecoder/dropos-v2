...
const PLANS = [
  ...
  },
  {
    ...
    features: [
      "Unlimited stores",
      "Unlimited products",
      "KIRO extended thinking",
      "KIRO vision (photo → listing)",
        ...
        <div className="max-w-5xl mx-auto grid md:grid-cols-3 gap-6">
          {PLANS.map((plan, i) => (
            <motion.div
              ...
              style={{
                background: plan.featured ? "rgba(107,53,232,0.08)" : "var(--bg-card)",
                border: plan.featured ? "2px solid var(--violet-500)" : "1px solid var(--border)",
              }}
            >
              {plan.featured && (
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 text-xs font-bold tracking-widest uppercase px-4 py-1 rounded-full text-white"
                  style={{ background: "var(--violet-500)" }}>
                  Most popular
                </div>
              )}
              <div className="mb-6">
                <h3 className="text-sm font-bold tracking-widest uppercase mb-2" style={{ color: plan.color }}>
                  {plan.name}
                </h3>
      ...
      <section className="py-16 px-6">
        <div className="max-w-xl mx-auto text-center">
          <p className="text-sm" style={{ color: "var(--text-secondary)" }}>
            All plans include a 14-day free trial of the Growth plan. No credit card required to start.
            Questions? <a href="mailto:hello@droposhq.com" className="underline" style={{ color: "var(--violet-400)" }}>hello@droposhq.com</a>
          </p>
        </div>
      </section>
    </div>
